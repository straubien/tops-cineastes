...
function openCourantDetail(courant){
  var cin=DATA.cineastes.filter(function(c){
    if(!c.courant)return false;
    return c.courant.split(' | ').some(function(m){return m.trim()===courant;});
  });
  cin.sort(function(a,b){return a.nom.localeCompare(b.nom,'fr')});
  var listDiv=document.createElement('div');
  listDiv.className='cineaste-list';
...
function renderStatistiques(){
  ...
  // Courants cinématographiques : un cinéaste peut en cumuler plusieurs
  // (valeurs séparées par « | »), chacun compte pour une occurrence.
  var courantCount={};
  DATA.cineastes.forEach(function(c){
    if(c.courant){
      c.courant.split(' | ').forEach(function(m){
  ...
  courKeys.forEach(function(k,i){
    ...
    row.innerHTML='<div class="'+rankClass+'">'+(i+1)+'</div>'
      +'<div class="ranking-name">'+escapeHtml(k)+'</div>'
      +'<div class="ranking-value">'+nb+' '+t('occurrences_in_ranking')+'</div>';
    row.setAttribute('data-courant',k);
    row.onclick=function(){openCourantDetail(this.getAttribute('data-courant'));};
