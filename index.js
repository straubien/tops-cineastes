...
function enterSite(){
  ...
  setTimeout(function(){splashEl.classList.add('hidden');splashEl.classList.remove('closing');},400);
}
...
function renderList(items){
  ...
  items.forEach(function(c,i){
    var row=document.createElement('div');row.className='cineaste-row row-in';
    row.style.animationDelay=(Math.min(i,15)*25)+'ms';
...
function closeFiche(){
  ...
  setTimeout(function(){overlay.classList.remove('visible');overlay.classList.remove('closing');},220);
}
...
function renderContributeurs(){
  ...
  list.innerHTML='';
  _contribs.forEach(function(c,i){
    var ratio=c.tops/maxTops;
    var tier=ratio>0.75?4:ratio>0.5?3:ratio>0.25?2:1;
    var div=document.createElement('div');
    div.className='contrib-bubble row-in';
    div.style.animationDelay=(Math.min(i,15)*25)+'ms';
...
function renderStatistiques(){
  ...
  ranked.forEach(function(c,i){
    ...
    var row=document.createElement('div');
    row.className='ranking-row row-in';
    row.style.animationDelay=(Math.min(i,15)*25)+'ms';
    row.setAttribute('data-fbid',c.fbid);
