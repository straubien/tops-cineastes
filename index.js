...
(function(){
  ...
  async function mtCheckNotifications(){
    ...
    try{
      ...
      var queries = [
        sbMT.from('submissions').select('id, status, parsed_json')
          .eq('contributor_id', mtCurrentContributor.id)
          .in('status', ['approved', 'rejected'])
          .is('seen_at', null),
        sbMT.from('cineaste_proposals').select('id, nom, prenom')
          .eq('contributor_id', mtCurrentContributor.id)
          .eq('status', 'approved')
          .is('seen_at', null),
        sbMT.from('comments').select('id, author_contributor_id, cineaste_nom')
      ...
      if(myOwnIds.length){
        queries.push(sbMT.from('comments').select('id, author_contributor_id, cineaste_nom')
  ...
  async function mtFlushPendingSeen(){
    ...
    for(var i=0;i<list.length;i++){
      ...
      if(res.error){
        ...
        // 0 ligne affectée sans erreur (RLS ou ligne supprimée) : on retente un nombre
        // limité de fois avant d'abandonner pour éviter une file qui grossit indéfiniment.
        p.attempts = (p.attempts || 0) + 1;
        if(p.attempts >= MT_PENDING_SEEN_MAX_ATTEMPTS){
          console.error('Abandon de la synchronisation seen_at après', p.attempts, 'tentatives pour', p.kind, p.id);
          mtClearPendingSeen(p.kind, p.id);
        } else {
          var current = mtGetPendingSeen();
          var idx = current.findIndex(function(c){ return c.kind === p.kind && c.id === p.id; });
          if(idx !== -1){ current[idx].attempts = p.attempts; try{ localStorage.setItem(mtPendingSeenKey(), JSON.stringify(current)); }catch(e){} }
        }
      }
    }
  }
  ...
  document.getElementById('prop-btn-submit').addEventListener('click', async function(){
    ...
    var dupRes = await sbMT.from('cineaste_proposals').select('id, nom').eq('status', 'pending').ilike('nom', nom);
