...
function navigate(page,skipHash){
  ...
  if(page==='photogramme'&&typeof window.pgOnNavigate==='function')window.pgOnNavigate();
...
window.addEventListener('popstate',function(e){
  ...
  } else if(hash==='#actualites'||hash==='#contributeurs'||hash==='#statistiques'||hash==='#mes-tops'||hash==='#photogramme'){
...
function tcActuFormatDateTime(iso){
  if(!iso)return'';
  var d=new Date(iso);
  if(isNaN(d.getTime()))return'';
...
function renderActualites(){
  ...
  Promise.all([
    ...
    tcWithRetryTimeout(function(){ return TC_SB.from('photogramme_sessions').select('*, contributors(display_name)').eq('status','scheduled').order('created_at',{ascending:false}).limit(20); }).then(function(r){return r.data||[];}).catch(function(){return[];})
  ]).then(function(results){
    var props=results[0],subs=results[1],coms=results[2],legacyKeys=new Set(results[3]),pgSessions=results[4];
    var items=[];
    props.forEach(function(p){
      ...
      items.push({date:p.submitted_at,html:'<span class="actu-icon">🎬</span> '+label});
    });
    ...
    pgSessions.forEach(function(s){
      var dn=(s.contributors&&s.contributors.display_name)||'?';
      var quand=tcActuFormatDateTime(s.scheduled_at);
      items.push({date:s.created_at,html:'<span class="actu-icon">🎞️</span> '+t('actu_photogramme_annonce',[escapeHtml(dn),'#'+s.id,quand]),pgSessionId:s.id});
    });
    ...
    feed.innerHTML=items.map(function(it){
      ...
      var cls=clickable||it.pgSessionId?' actu-item-clickable':'';
      var attr=clickable?' data-action="tc-actu-open" data-cineaste="'+escapeHtml(it.cineaste)+'"':(it.pgSessionId?' data-action="tc-actu-open-photogramme" data-pg-session="'+it.pgSessionId+'"':'');
...
window.tcAfterLangChange = function(){
  ...
  else if(p==='photogramme'&&typeof window.pgOnNavigate==='function'){window.pgOnNavigate();}
};
...
document.getElementById('nav-photogramme').addEventListener('click',function(){navigate('photogramme');});
document.getElementById('mt-page-back').addEventListener('click',function(){navigate('index');});
document.getElementById('pg-page-back').addEventListener('click',function(){navigate('index');});
...
(function(){
  ...
  async function mtFlushPendingSeen(){
    ...
    for(var i=0;i<list.length;i++){
      var p = list[i];
      var res = await sbMT.rpc('mark_notification_seen', { p_kind: p.kind, p_id: p.id });
      if(res.error){
        ...
        // RPC a retourné false (ligne déjà marquée vue, ou non trouvée) : on retente un
        // nombre limité de fois avant d'abandonner pour éviter une file qui grossit indéfiniment.
  ...
  async function mtDismissNotification(n, item){
    ...
    for(var attempt = 0; attempt < 3 && !success; attempt++){
      try{
        var res = await sbMT.rpc('mark_notification_seen', { p_kind: n.kind, p_id: n.id });
