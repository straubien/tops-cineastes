...
document.getElementById('nav-comparaison').addEventListener('click',function(){navigate('comparaison');});
...
//    côtés) — le premier match déclenche un gros bonus fixe, chaque match
//    supplémentaire n'ajoute qu'un petit complément (pas de simple linéaire,
//    pour qu'un SEUL point commun compte déjà énormément).
//  - Autre↔Autre (•) : un cinéaste/film apprécié des deux côtés (sans être
//    un top favori) reflète une vraie proximité de goût — remonté proche du
//    niveau favori↔favori plutôt que traité comme le signal le plus faible.
//  - Favori↔Autre (★, un seul côté) : palier intermédiaire, inchangé.
var COMPARE_FAVFAV_FIRST_BONUS=8;   // bonus au 1er favori↔favori commun
var COMPARE_FAVFAV_EXTRA=2;         // par favori↔favori commun supplémentaire
var COMPARE_OTHEROTHER_WEIGHT=3;    // par autre↔autre commun
var COMPARE_FAVOTHER_WEIGHT=1.5;    // par favori↔autre (un seul côté) commun
...
function compareTwoIndexes(idxA,idxB){
  ...
  var score=(nFavFav>0?COMPARE_FAVFAV_FIRST_BONUS+(nFavFav-1)*COMPARE_FAVFAV_EXTRA:0)
