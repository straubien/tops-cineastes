...
// ── Hommages — diaporama des cinéastes décédés dans l'année en cours ──
// Un cinéaste (ou un membre d'un duo) est retenu si sa colonne "deces"
// vaut l'année en cours et "vivant" vaut false. Pour un duo, les deux
...
var HOMMAGES_SLIDES=[],HOMMAGES_INDEX=0,HOMMAGES_TIMER=null;
...
function tcHommagesStartAutoplay(){
  ...
  if(HOMMAGES_SLIDES.length<2)return;
...
function renderHommages(){
  ...
  var slides=[];
  DATA.cineastes.forEach(function(c){
    var qualifies=false,decesTri=null;
    if(c.duo&&Array.isArray(c.naissance)){
      var deces=Array.isArray(c.deces)?c.deces:[null,null];
      var vivant=Array.isArray(c.vivant)?c.vivant:[null,null];
      for(var i=0;i<2;i++){
        if(deces[i]===currentYear&&vivant[i]===false){qualifies=true;decesTri=currentYear;}
      }
    } else if(c.deces===currentYear&&c.vivant===false){
      qualifies=true;decesTri=c.deces;
    }
    if(qualifies){
      slides.push({
        cineaste:c,
  ...
  slides.sort(function(a,b){
    if(b.deces!==a.deces)return (b.deces||0)-(a.deces||0);
    return a.cineaste.nom.localeCompare(b.cineaste.nom,'fr');
  });
  HOMMAGES_SLIDES=slides;
  HOMMAGES_INDEX=0;
  if(!slides.length){
    container.innerHTML='<div class="empty-msg">'+t('hommages_vide')+'</div>';
    tcHommagesStopAutoplay();
    return;
  }
  ...
  container.onmouseleave=function(){tcHommagesStartAutoplay();};
}
