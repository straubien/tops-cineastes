...
// Chemin d'un portrait local (dossier /portraits) à partir du nom "NOM, Prénom"
function localPortraitPath(nom){
  if(!nom) return null;
  ...
  if(/bergman/i.test(nom)&&/marta/i.test(nom)) return 'portraits/portrait-Bergman, Marta.jpg';
  if(/bergman/i.test(nom)&&/ingmar/i.test(nom)) return 'portraits/portrait-Bergman, Ingmar.jpg';
...
function portraitFallbackHtml(nom,cls){
  var path=localPortraitPath(nom);
...
(function(){
  ...
  async function mtSubmitThematique(){
    ...
    btn.disabled = false; btn.textContent = t('thematique_btn_submit');
    if(res.error){ alert(t('mt_err_submit') + friendlyError(res.error)); return; }
