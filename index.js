...
function tcRefreshOnlineBadges(){
  ...
  tcRenderOnlineUsersBar();
}
// Bandeau « cinéphiles en ligne » affiché en bas de chaque page, alimenté par
// le canal de présence Supabase Realtime (aucune requête supplémentaire).
function tcRenderOnlineUsersBar(){
  var bar=document.getElementById('online-users-bar');
  var listEl=document.getElementById('online-users-list');
  if(!bar||!listEl)return;
  var names=CONTRIB_DATA.filter(function(c){return TC_ONLINE_IDS.has(String(c.id));})
    .map(function(c){return formatContribNamePlain(c.display_name);})
...
function loadData(){
  ...
  Promise.all([
    ...
    tcWithRetryTimeout(function(){ return tcLoadCourants(TC_SB); }).catch(function(){return [];}),
  ]).then(function(results){
  var cineastes=results[0]||[];
  var muzard=results[1];
  var cnudde=results[2];
  var courants=results[3]||[];
...
function enterSite(){
  ...
  // Mémorisé dans localStorage et non sessionStorage : le splash est un
  // moment d'identité, pas un péage à repayer à chaque nouvel onglet.
  try{ localStorage.setItem('tc-entered','1'); }catch(e){}
  try{ sessionStorage.setItem('tc-entered','1'); }catch(e){}
...
// ── RÉSOLUTION D'UN NOM DE CINÉASTE ────────────────────────────────
// Les trois sources n'écrivent pas les noms de la même façon :
// cineaste_coeur donne « Pier Paolo Pasolini », cineaste_autres donne
// « PASOLINI, Pier Paolo », et l'index « PASOLINI, Pier Paolo ». Une
// comparaison de chaînes échouerait sur la moitié des cas.
// On compare donc l'ENSEMBLE DES MOTS du nom, sans tenir compte de
// l'ordre, de la casse, des accents ni de la ponctuation.
// Un nom ambigu (« Bergman » seul, alors que l'index en compte deux)
// ne renvoie rien : mieux vaut un nom inerte qu'un lien vers le mauvais
// cinéaste.
...
function tcResoudreCineaste(saisie){
  ...
  // 1. Correspondance exacte de l'ensemble des mots
  var cle = jetons.join('|');
  if(idx.parSig[cle] && idx.parSig[cle].length === 1) return idx.parSig[cle][0];

  // 2. Sous-ensemble unique — résout les duos, « Straub/Huillet »
  //    étant contenu dans « STRAUB, Jean-Marie & HUILLET, Danièle ».
  var candidats = [], k, i;
  for(i=0;i<idx.cles.length;i++){
    ...
    if(tous){
      ...
      if(candidats.length > 1) return null; // ambigu : on s'arrête
    }
  }
  return candidats.length === 1 ? candidats[0] : null;
}

// Ouvre la fiche d'un cinéaste depuis son nom exact de l'index.
...
function renderProfilFavs(favs){
  ...
  favs.forEach(function(f){
    ...
    // Le premier favori n'est plus arbitrairement plus gros que les autres :
    // la grille est régulière, comme des planches alignées.
    item.className='profil-fav-item';
    var cible=tcResoudreCineaste(f);
    if(cible){
      item.classList.add('profil-cin-lien');
      item.setAttribute('data-cineaste',cible);
      item.setAttribute('role','button');
      item.setAttribute('tabindex','0');
    }
    var candidates=p.candidates.slice();
    item.innerHTML='<img class="profil-fav-portrait" src="'+escapeHtml(candidates.shift())+'" alt="'+escapeHtml(f)+'" loading="lazy">'
      +'<div class="profil-fav-initials" style="display:none">'+escapeHtml(p.initials)+'</div>'
...
function renderProfil(name){
  ...
  // La taille du nom était cherchée par une boucle décroissante de 40 à
  // 18px, qui forçait jusqu'à vingt-deux recalculs de mise en page à
  // chaque affichage de profil. Une taille fluide fait le même travail
  // dans la feuille de style, sans aucun recalcul.
  nameEl.style.fontSize='';
  var imp=IMPORTED_COUNTS[name]||{tops:0,films:0};
  var statHtml='<span>'+t('tops_postes',imp.tops)+'</span>'
    +'<span class="profil-stat-sep">·</span>'
    +'<span>'+t('une_films',imp.films)+'</span>';
...
function renderProfilPortrait(name,cin){
  ...
  // 4) Courants & genres
  var courKeys=Object.keys(courantCount).sort(function(a,b){return courantCount[b]-courantCount[a];});
  if(courKeys.length){
    var courHtml=courKeys.slice(0,15).map(function(k){
      return '<span class="pp-mention"><span class="pp-mention-lab">'+escapeHtml(tcCourantLabel(Number(k)))+'</span><b>'+courantCount[k]+'</b></span>';
    }).join('');
    html+='<div class="pp-card"><div class="pp-card-head">'+t('pp_courants')+'</div>'
      +'<div class="pp-mentions">'+courHtml+'</div>'
      +'<div class="pp-note">'+t('pp_courants_note')+'</div></div>';
  }
...
function buildAlpha(){
  ...
  // L'alphabet est affiché en entier : une lettre sans cinéaste reste
  // visible mais inerte. Auparavant elle disparaissait, et la barre
  // changeait de largeur selon les données.
  'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').forEach(function(l){
    var items=getLetterData(l);
    var vide=!items.length;
    var col=document.createElement('div');
    col.className='alpha-col '+(vide?'no-data':'has-data')+((!vide&&l===currentLetter)?' active':'');
    col.innerHTML='<div class="alpha-char">'+l+'</div>';
    if(!vide){
      col.setAttribute('role','button');
      col.setAttribute('tabindex','0');
      col.setAttribute('aria-label',l);
      col.onclick=function(){selectLetter(l)};
      col.onkeydown=function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();selectLetter(l);}};
    }
    bar.appendChild(col);
  });
}
...
function renderList(items){
  ...
  items.forEach(function(c){
    ...
    // Ligne de métadonnées : drapeau(x), dates, puis jusqu'à deux courants.
    // Les dates y restent visibles en mobile, où elles étaient masquées.
    var flagsHtml=(typeof tcFlagHtml==='function')
      ? (tcFlagHtml(c.pays,'c-flag')+tcFlagHtml(c.pays2,'c-flag')) : '';
    var courantsHtml='';
    if(typeof tcCourantIds==='function'){
      courantsHtml=tcCourantIds(c).slice(0,2).map(function(id){
        return '<span class="c-courant">'+escapeHtml(tcCourantLabel(id))+'</span>';
      }).join('');
    }
    var metaInner=flagsHtml+datesHtml+courantsHtml;
    var metaHtml=metaInner?('<div class="c-meta">'+metaInner+'</div>'):'';
    row.innerHTML='<div class="c-bar"></div>'
      +photoHtml
      +'<div class="c-info">'
        +'<div class="c-name">'+formatNom(c.nom,false)+'</div>'
        +metaHtml
      +'</div>'
...
// ── BANDEAU D'ACCUEIL — dernier top validé ─────────────────────
// Une requête unique, limitée à une ligne : l'accueil ne charge pas le
// fil d'actualités complet pour afficher une seule information. Le
// bandeau reste masqué tant qu'on n'a rien à y mettre, et toute erreur
// est silencieuse : ce n'est pas un contenu essentiel.
function tcRenderUne(){
  var el=document.getElementById('tc-une');
  if(!el) return;
  tcWithRetryTimeout(function(){
    return TC_SB.from('submissions')
      .select('parsed_json, reviewed_at, submitted_at, contributors(display_name)')
      .eq('status','approved')
    ...
    var row=res.data[0];
    var pj=row.parsed_json||{};
    var cineaste=pj.cineaste;
    if(!cineaste) return;
    var nbFilms=Array.isArray(pj.films)?pj.films.length:0;
    var auteur=(row.contributors&&row.contributors.display_name)||'';
    var meta=[];
    if(auteur) meta.push(escapeHtml(formatContribNamePlain(auteur)));
    if(nbFilms) meta.push(t('une_films',nbFilms));
    el.innerHTML='<span class="tc-une-lab">'+t('une_dernier_top')+'</span>'
      +'<span class="tc-une-nom">'+formatNom(cineaste)+'</span>'
      +(meta.length?'<span class="tc-une-meta">'+meta.join(' \u00b7 ')+'</span>':'');
    el.setAttribute('role','button');
    el.setAttribute('tabindex','0');
    el.style.display='';
    var ouvrir=function(){
      if(!DATA)return;
      var f=DATA.cineastes.find(function(x){return x.nom===cineaste;});
      if(f)openFiche(f);
    };
    el.onclick=ouvrir;
    el.onkeydown=function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();ouvrir();}};
  }).catch(function(){ /* bandeau facultatif : aucun message d'erreur */ });
}
DATA_READY.then(function(){ tcRenderUne(); });
...
function openFiche(c){
    ...
    CNUDDE_DATA[c.nom]:SUPABASE_TOPS[b]?SUPABASE_TOPS[b][c.nom]:null)||[];
      ...
      if(hasFilms){
        ...
        films.forEach(function(f){
          ...
          if(f.note){var sn=document.createElement('span');sn.className='film-note';sn.textContent='('+f.note+')';s1.appendChild(sn);}
          li.appendChild(s1);
  ...
  if(ficheCourantIds.length){
    var badgesHtml=ficheCourantIds.map(function(id){
      return '<span class="fiche-mention-badge">'+escapeHtml(tcCourantLabel(id))+'</span>';
    }).join('');
    ficheCourantHtml='<div class="fiche-courant">'+badgesHtml+'</div>';
  }

  // Légende de la plaque : drapeau(x), nationalité(s), puis les dates.
  // Ces informations quittent le bloc du nom pour se ranger sous la
  // photographie, comme la légende d'une illustration de revue.
  var paysLabels=[];
  if(typeof tcCountryLabel==='function'){
    [c.pays,c.pays2].forEach(function(code){
      var lab=code?tcCountryLabel(code):'';
      if(lab&&paysLabels.indexOf(lab)===-1)paysLabels.push(lab);
    });
  }
  var ficheDatesTxt=formatDates(c);
  var legendeLignes=[];
  if(paysLabels.length)legendeLignes.push(escapeHtml(paysLabels.join(' / ')));
  if(ficheDatesTxt)legendeLignes.push(ficheDatesTxt);
  var ficheLegendeHtml=(ficheFlagsHtml||legendeLignes.length)
    ? '<div class="fiche-legende">'+ficheFlagsHtml+legendeLignes.join('<br>')+'</div>'
    : '';
  ...
  document.getElementById('fiche-content').innerHTML=
    '<div class="fiche-header">'
      ...
      +'<div class="fiche-header-top">'
        +'<div class="fiche-plaque">'+fichePhotoHtml+ficheLegendeHtml+'</div>'
    ...
    +'<div class="fiche-body">'
      +'<div class="fiche-barre">'
        +'<span class="fiche-stat-number">'+nb+'</span>'
        +'<span class="fiche-stat-label">'+t('tops_postes_par')+'</span>'
      +'</div>'
      +'<div class="fiche-contribs">'
...
function tcNotifText(n){
  ...
  switch(n.type){
    ...
    case 'proposal_courant_approved': return t('notif_prop_courant',[cine,(n.data&&n.data.courant)||'']);
...
function renderActualites(){
  ...
  Promise.all([
    ...
    props.forEach(function(p){
      ...
      var icon=approved?'✅':'🎬';
      var item={date:p.reviewed_at||p.submitted_at,html:'<span class="actu-icon">'+icon+'</span> '+label};
    ...
    courantProps.forEach(function(p){
      ...
      items.push({date:p.reviewed_at||p.submitted_at,html:'<span class="actu-icon">🏷️</span> '+t('actu_courant_valide',[escapeHtml(dn),formatNom(p.cineaste_nom||''),'<span class="actu-courant-name">'+escapeHtml(tcCourantLabelFromName(p.courant||''))+'</span>']),cineaste:p.cineaste_nom});
    });
...
function renderStatistiques(){
  ...
  var courKeys=Object.keys(courantCount).sort(function(a,b){
    var ya=(_TC_COURANTS_CACHE&&_TC_COURANTS_CACHE[a]&&_TC_COURANTS_CACHE[a].annee_debut)||Infinity;
    var yb=(_TC_COURANTS_CACHE&&_TC_COURANTS_CACHE[b]&&_TC_COURANTS_CACHE[b].annee_debut)||Infinity;
    return ya-yb;
  });
  var totalCourants=courKeys.length;

  // Trois sous-libellés sont retirés plutôt que masqués en CSS :
  //  - « en moyenne par contributeur » faisait doublon avec la tuile
  //    « Nombre de tops moyen/cinéphile » juste dessous ;
  //  - « cinéastes décédés indexés » était rattaché à la tuile des
  //    cinéphiles, à laquelle il ne se rapporte pas ;
  //  - « cumulés dans tous les tops importés » n'ajoutait rien au libellé.
  // Le sous-libellé des cinéastes sans top devient une part de l'index,
  // là où il ne faisait que répéter son propre intitulé.
  var pctSansTops=totalC?Math.round(st/totalC*100):0;
  document.getElementById('stats-hero').innerHTML=
    mkHero(totalC,t('stat_cin'),t('stat_en_activite',enA))
    +mkHero(tt,t('stat_tops_postes'))
    +mkHero(totalContribs,t('stat_contrib'))
    +mkHero(st,t('stat_cin_sans_tops'),t('stat_sans_tops_pct',pctSansTops))
    +mkHero(totalFilms,t('stat_films'))
    +mkHero(totalCourants,t('stat_courants'));
  ...
  // Le classement mélangeait aléatoirement la liste avant de la trier :
  // le tri étant stable, les ex aequo gardaient cet ordre au hasard et le
  // podium changeait d'une visite à l'autre. Les égalités sont désormais
  // départagées par ordre alphabétique, donc le classement est reproductible.
  var cinTopCount={};
  DATA.cineastes.forEach(function(c){cinTopCount[c.nom]=(c.tops_contributeurs||[]).length;});
  var ranked=DATA.cineastes
    .filter(function(c){return(cinTopCount[c.nom]||0)>0;})
    .sort(function(a,b){
      var ecart=(cinTopCount[b.nom]||0)-(cinTopCount[a.nom]||0);
  ...
  var courRankEl=document.getElementById('ranking-courants');
  courRankEl.innerHTML='';
  courRankEl.className='courants-timeline';
  var courPrevDebut=null;
  courKeys.forEach(function(k){
    var row=document.createElement('div');
    row.className='courant-tl-item';
    var courFlag=tcCourantFlagHtml(Number(k),'ranking-flag');
    var courYears=tcCourantYears(Number(k));
    var courYearsHtml=courYears?'<span class="courant-tl-period">'+courYears+'</span>':'';
    var debut=(_TC_COURANTS_CACHE&&_TC_COURANTS_CACHE[k]&&_TC_COURANTS_CACHE[k].annee_debut)||'';
    var debutHtml=(debut!==''&&debut===courPrevDebut)?'':debut;
    courPrevDebut=debut;
    row.innerHTML='<div class="courant-tl-year">'+debutHtml+'</div>'
      +'<div class="courant-tl-dot"></div>'
      +'<div class="courant-tl-content">'
        +'<div class="courant-tl-name">'+escapeHtml(tcCourantLabel(Number(k)))+courFlag+'</div>'
        +courYearsHtml
      +'</div>';
    row.setAttribute('data-courant',k);
    row.onclick=function(){openCourantDetail(Number(this.getAttribute('data-courant')));};
    courRankEl.appendChild(row);
  });
...
window.tcAfterLangChange = function(){
  ...
  if(p==='index'){tcRenderUne();if(document.getElementById('search-input').value.trim()){filterList();}else{selectLetter(currentLetter);}}
...
// Les cinéastes du profil, favoris comme autres, ouvrent leur fiche.
// Délégation posée une fois sur les deux conteneurs, qui sont statiques.
['profil-favs-grid','profil-autres-cineaste'].forEach(function(id){
  var zone=document.getElementById(id);
  if(!zone)return;
  zone.addEventListener('click',function(e){
    var el=e.target.closest('.profil-cin-lien');
    if(el)tcOuvrirFicheParNom(el.getAttribute('data-cineaste'));
  });
  zone.addEventListener('keydown',function(e){
    if(e.key!=='Enter'&&e.key!==' ')return;
    var el=e.target.closest('.profil-cin-lien');
    if(el){e.preventDefault();tcOuvrirFicheParNom(el.getAttribute('data-cineaste'));}
  });
});
// ═══════════════════════════════════════════════════════════════
...
var _compareAffinityFormula='min';
...
// localStorage d'abord (visites suivantes, y compris un autre jour),
// sessionStorage ensuite pour rester compatible avec l'onglet en cours.
(function(){
  var deja=false;
  try{ deja=!!localStorage.getItem('tc-entered'); }catch(e){}
  if(!deja){ try{ deja=!!sessionStorage.getItem('tc-entered'); }catch(e){} }
  if(deja) enterSite();
})();
...
function openFicheThematique(themeNom){
  ...
  rows.slice().sort(function(a,b){
    ...
    if(hasFilms){
      ...
      films.forEach(function(f){
        ...
        if(f && f.note){ var sn = document.createElement('span'); sn.className = 'film-note'; sn.textContent = '(' + f.note + ')'; s1.appendChild(sn); }
        li.appendChild(s1);
  ...
  if(headerTop){
    var plaqueT = document.createElement('div');
    plaqueT.className = 'fiche-plaque';
    plaqueT.appendChild(fichePhotoEl);
    headerTop.insertBefore(plaqueT, headerTop.firstChild);
  }
...
(function(){
  ...
  function mtGetExistingCourants(){
    ...
    var out = (DATA && DATA.courants ? DATA.courants : []).map(function(row){
      return { value: row.nom_fr, label: (lang === 'en' && row.nom_en) ? row.nom_en : row.nom_fr };
    });
  ...
  function mtInitCourantTab(){
    ...
    sel.innerHTML = '<option value="">' + t('mt_courant_select_default') + '</option>'
      + existants.map(function(c){ return '<option value="' + escapeHtml(c.value) + '">' + escapeHtml(c.label) + '</option>'; }).join('');
  }
