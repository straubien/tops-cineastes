...
function renderProfilPortrait(name,cin){
  ...
  // Balayage des films : années, décennies, extrêmes
  var decennies={},anneesDatees=0,minY=null,maxY=null,minFilm=null,maxFilm=null;
  items.forEach(function(it){
    (it.films||[]).forEach(function(f){
      ...
      if(minY===null||y<minY){minY=y;minFilm=f;}
      if(maxY===null||y>maxY){maxY=y;maxFilm=f;}
    });
  });
  ...
  cin.forEach(function(c){
    if(c.vivant===true)enActivite++;
    else if(c.vivant===false)disparus++;
  ...
  var periode=(minY!==null&&maxY!==null)?(minY===maxY?String(minY):(minY+'<span class="pp-tile-sep">–</span>'+maxY)):'—';
  html+='<div class="pp-tiles">'
    ...
    +'<div class="pp-tile"><div class="pp-tile-num">'+(moyFilms?moyFilms.toFixed(1).replace('.',','):'—')+'</div><div class="pp-tile-lab">'+t('pp_films_par_cin')+'</div></div>'
    +'<div class="pp-tile"><div class="pp-tile-num">'+periode+'</div><div class="pp-tile-lab">'+t('pp_periode')+'</div></div>'
    +'</div>';

  // 2) Époque de prédilection (par décennie)
  ...
  if(courKeys.length){
    var courHtml=courKeys.slice(0,12).map(function(k){
  ...
  // 5) Duo : vivants / disparus + extrêmes
  var duoLeft='',duoRight='';
  var pctActif=null;
  if(statutConnu>0){
    pctActif=Math.round(enActivite/statutConnu*100);
    var pctDisp=100-pctActif;
    var liveW=pctActif,deadW=pctDisp;
    duoLeft='<div class="pp-card pp-mini"><div class="pp-card-head">'+t('pp_vivants')+'</div>'
      +'<div class="pp-split">'
      +(liveW>0?'<div class="pp-split-live" style="width:'+liveW+'%">'+(liveW>=12?pctActif+'%':'')+'</div>':'')
      +(deadW>0?'<div class="pp-split-dead" style="width:'+deadW+'%">'+(deadW>=12?pctDisp+'%':'')+'</div>':'')
      +'</div>'
  ...
  if(minFilm||maxFilm){
    duoRight='<div class="pp-card pp-mini"><div class="pp-card-head">'+t('pp_extremes')+'</div>';
    if(minFilm)duoRight+='<div class="pp-extreme"><span class="pp-ex-lab">'+t('pp_film_ancien')+'</span><span class="pp-ex-val">'+escapeHtml(minFilm.titre||'')+' <em>('+minY+')</em></span></div>';
    if(maxFilm)duoRight+='<div class="pp-extreme"><span class="pp-ex-lab">'+t('pp_film_recent')+'</span><span class="pp-ex-val">'+escapeHtml(maxFilm.titre||'')+' <em>('+maxY+')</em></span></div>';
    duoRight+='</div>';
  }
  if(duoLeft||duoRight){
    html+='<div class="pp-duo">'+duoLeft+duoRight+'</div>';
  }
