...
function renderActualites(){
  ...
  Promise.all([
    ...
    subs.forEach(function(s){
      ...
      var validatedBy=s.reviewed_by?'<span class="actu-validated-by">'+t('actu_valide_par',[escapeHtml(tcAuthorLabel(s.reviewed_by))])+'</span>':'';
      items.push({date:s.reviewed_at||s.submitted_at,html:tcActuAvatarHtml(s.contributor_id,dn)+' '+label+validatedBy,cineaste:cin});
    });
...
function renderContributeurs(){
  ...
  _contribs=CONTRIB_DATA
    ...
    .map(function(c){
      ...
      return{name:dn,tops:nb,admin:!!c.is_admin};
    });
  ...
  _contribs.forEach(function(c){
    ...
    div.innerHTML='<div class="contrib-bubble-band"></div>'
      ...
      +(c.admin?'<span class="contrib-admin-badge">Admin</span>':'')
...
function openContribDetail(name){
  ...
  var _contribInfo=CONTRIB_DATA.find(function(c){return c.json_name===name;});
  var adminBadgeHtml=(_contribInfo&&_contribInfo.is_admin)?'<div class="fiche-admin-badge"><span class="contrib-admin-badge">Admin</span></div>':'';
