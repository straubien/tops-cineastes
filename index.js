...
function renderActualites(){
  ...
  Promise.all([
    ...
    courantProps.forEach(function(p){
      ...
      items.push({date:p.reviewed_at||p.submitted_at,html:'<span class="actu-icon">🏷️</span> '+t('actu_courant_valide',[escapeHtml(dn),formatNom(p.cineaste_nom||''),'<span class="actu-courant-name">'+escapeHtml(tcCourantLabelFromName(p.courant||''))+'</span>']),cineaste:p.cineaste_nom,type:'courants'});
    });
