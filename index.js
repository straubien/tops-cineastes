...
function renderProfilPortrait(name,cin){
  ...
  cin.forEach(function(c){
    ...
    if(c.courant){
      c.courant.split(' | ').forEach(function(m){
        var k=m.trim();
        if(k)courantCount[k]=(courantCount[k]||0)+1;
      });
    }
  });
  ...
  if(courKeys.length){
    var courHtml=courKeys.slice(0,15).map(function(k){
      return '<span class="pp-mention"><span class="pp-mention-lab">'+escapeHtml(k)+'</span><b>'+courantCount[k]+'</b></span>';
    }).join('');
...
function openFiche(c){
  ...
  if(c.courant){
    var badgesHtml=c.courant.split(' | ').map(function(m){
      return '<span class="fiche-mention-badge">'+escapeHtml(m.trim())+'</span>';
    }).join('');
...
function renderActualites(){
  ...
  Promise.all([
    ...
    courantProps.forEach(function(p){
      ...
      items.push({date:p.reviewed_at||p.submitted_at,html:'<span class="actu-icon">🎭</span> '+t('actu_courant_valide',[escapeHtml(dn),formatNom(p.cineaste_nom||''),escapeHtml(p.courant||'')]),cineaste:p.cineaste_nom});
    });
...
// ── Classement TMDB : 50 films les plus vus ─────────────────────
var _topFilmsTmdbCache = null;
// Charge toutes les lignes auto/manuel non exclues du classement (paginé).
async function _loadTopFilmsRows(offset, pageSize){
  var res = await tcWithRetryTimeout(function(){
    return TC_SB.from('film_tmdb_matches')
      .select('tmdb_id, tmdb_titre, tmdb_annee')
      .in('statut', ['auto', 'manuel'])
      .eq('exclu_classement', false)
      .not('tmdb_id', 'is', null)
      .range(offset, offset + pageSize - 1);
  });
  if(res.error) return [];
  var rows = res.data || [];
  if(rows.length === pageSize){
    var more = await _loadTopFilmsRows(offset + pageSize, pageSize);
    return rows.concat(more);
  }
  return rows;
}
async function renderTopFilmsTmdb(){
  var el = document.getElementById('ranking-top-films-tmdb');
  if(!el) return;
  if(_topFilmsTmdbCache === null){
    el.innerHTML = '<div class="empty-msg" style="padding:1rem">…</div>';
    var rows = await _loadTopFilmsRows(0, 1000);
    // Grouper par tmdb_id et compter
    var counts = {}, meta = {};
    rows.forEach(function(row){
      var id = row.tmdb_id;
      counts[id] = (counts[id] || 0) + 1;
      if(!meta[id]) meta[id] = { tmdb_titre: row.tmdb_titre, tmdb_annee: row.tmdb_annee };
    });
    _topFilmsTmdbCache = Object.keys(counts)
      .map(function(id){ return { tmdb_id: Number(id), tmdb_titre: meta[id].tmdb_titre, tmdb_annee: meta[id].tmdb_annee, nb: counts[id] }; })
      .sort(function(a, b){ return b.nb - a.nb; })
      .slice(0, 50);
  }
  el.innerHTML = '';
  if(!_topFilmsTmdbCache.length){
    el.innerHTML = '<div class="empty-msg">'+t('ranking_top_films_tmdb_vide')+'</div>';
    return;
  }
  _topFilmsTmdbCache.forEach(function(f, i){
    var rankClass = i < 10 ? 'ranking-rank top3' : 'ranking-rank';
    var annee = f.tmdb_annee ? ' <span style="font-size:16px;font-weight:400;opacity:.6">('+escapeHtml(String(f.tmdb_annee))+')</span>' : '';
    var row = document.createElement('div');
    row.className = 'ranking-row';
    row.style.cursor = 'default';
    row.innerHTML = '<div class="'+rankClass+'">'+(i+1)+'</div>'
      +'<div class="ranking-name">'+escapeHtml(f.tmdb_titre||'—')+annee+'</div>'
      +'<div class="ranking-value">'+f.nb+' '+t('citations_in_ranking')+'</div>';
    el.appendChild(row);
  });
}
...
(function(){
  ...
  // Liste dédupliquée des courants déjà répertoriés (déduite des fiches
  // cinéastes chargées côté client) — aucune saisie libre n'est proposée,
  // il est donc impossible de créer un nouveau courant depuis ce formulaire.
  function mtGetExistingCourants(){
    var set = {};
    var out = [];
    (DATA && DATA.cineastes ? DATA.cineastes : []).forEach(function(c){
      if(!c.courant) return;
      c.courant.split(' | ').forEach(function(m){
        var v = m.trim();
        if(v && !set[v.toLowerCase()]){ set[v.toLowerCase()] = true; out.push(v); }
      });
    });
  ...
  function mtInitCourantTab(){
    ...
    sel.innerHTML = '<option value="">' + t('mt_courant_select_default') + '</option>'
      + existants.map(function(c){ return '<option value="' + escapeHtml(c) + '">' + escapeHtml(c) + '</option>'; }).join('');
  }
  ...
  async function mtLoadMyCourantProposals(){
    ...
    res.data.forEach(function(p){
      ...
      card.innerHTML = '<div class="mt-prev-card-head">'
        + '<span class="mt-prev-card-cin">' + escapeHtml(formatNom(p.cineaste_nom)) + ' — ' + escapeHtml(p.courant) + '</span>'
        + '<span class="mt-prev-status ' + statusCls + '">' + statusLabel + '</span>'
        + '</div>';
      listEl.appendChild(card);
    });
