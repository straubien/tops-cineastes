...
function tcSubmitComment(uid){
  ...
  ta.disabled=true;
  tcWithRetryTimeout(function(){return TC_SB.from('comments').insert(payload).select('*').single();}).then(function(res){
    ta.disabled=false;
    if(res.error){tcShowBanner('tc-error-banner',t('tc_cm_erreur_envoi'),'#b3261e');return;}
    if(!res.data){tcShowBanner('tc-error-banner',t('tc_cm_erreur_envoi'),'#b3261e');return;}
    st.rows.push(res.data);
    st.replyTo=null;
    tcRenderComments(uid);
  },function(){ta.disabled=false;tcShowBanner('tc-error-banner',t('tc_cm_erreur_envoi'),'#b3261e');});
}
...
function tcSaveEdit(uid,commentId){
  ...
  tcWithRetryTimeout(function(){return TC_SB.from('comments').update({body:body,updated_at:new Date().toISOString()}).eq('id',commentId).select('*').single();}).then(function(res){
    ...
    st.editingId=null;
    tcRenderComments(uid);
  },function(){tcShowBanner('tc-error-banner',t('tc_cm_erreur_envoi'),'#b3261e');});
}
...
function tcDeleteComment(uid,commentId){
  ...
  tcWithRetryTimeout(function(){return TC_SB.from('comments').delete().eq('id',commentId).select('id');}).then(function(res){
    if(res.error){tcShowBanner('tc-error-banner',t('tc_cm_erreur_envoi'),'#b3261e');return;}
    if(!res.data||!res.data.length){tcShowBanner('tc-error-banner',t('tc_cm_erreur_envoi'),'#b3261e');return;}
...
(function(){
  ...
  // Auth listener #1 : gère la nav connecté/déconnecté et le chargement initial du profil (sbAuth)
...
(function(){
  ...
  // Auth listener #2 : initialise le contributeur pour mes-tops (sbMT, instance séparée de sbAuth)
