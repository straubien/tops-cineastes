...
function renderProfilPortrait(name,cin){
  ...
  if(catKeys.length){
    var catHtml=catKeys.slice(0,15).map(function(k){
      return '<span class="pp-mention pp-mention-categorie"><span class="pp-mention-lab">'+escapeHtml(tcCourantLabel(Number(k)))+'</span><b>'+courantCount[k]+'</b></span>';
    }).join('');
    html+='<div class="pp-card"><div class="pp-card-head">'+t('pp_categories')+'</div>'
      +'<div class="pp-mentions">'+catHtml+'</div>'
      +'<div class="pp-note">'+t('pp_categories_note')+'</div></div>';
  }
...
function openFiche(c){
  ...
  // Les catégories (transversales) doivent toujours apparaître avant les
  // courants (mouvements datés) sur la fiche, quel que soit l'ordre de
  // saisie des champs courant/courant2/courant3. Tri stable : on ne change
  // que l'ordre relatif catégorie/courant, jamais l'ordre au sein d'un même type.
  ficheCourantIds=ficheCourantIds.slice().sort(function(a,b){
    var aRang=tcCourantType(a)==='categorie'?0:1;
    var bRang=tcCourantType(b)==='categorie'?0:1;
    return aRang-bRang;
  });
