...
function portraitFallbackHtml(nom,cls){
  var path=tcLocalPortraitPath(nom);
  var emptyDiv='<div class="'+cls+' '+cls+'-empty"></div>';
  if(!path) return emptyDiv;
  return '<img class="'+cls+' photo-portrait-fallback" src="'+escapeHtml(path)+'" alt="" loading="lazy"'
    +' data-empty-class="'+escapeHtml(cls+' '+cls+'-empty')+'">';
}
// Attache le remplacement par le div vide sur les portraits locaux introuvables.
// (Les handlers inline sont interdits par la CSP, on câble donc après insertion.)
