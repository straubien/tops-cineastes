...
var HOMMAGES_SLIDES=[],HOMMAGES_INDEX=0,HOMMAGES_TIMER=null,HOMMAGES_PLAYING=true;
...
function tcHommagesStopAutoplay(){
  if(HOMMAGES_TIMER){clearInterval(HOMMAGES_TIMER);HOMMAGES_TIMER=null;}
}
function tcHommagesStartAutoplay(){
  tcHommagesStopAutoplay();
  if(!HOMMAGES_PLAYING||HOMMAGES_SLIDES.length<2)return;
  HOMMAGES_TIMER=setInterval(function(){
    tcHommagesGoTo(HOMMAGES_INDEX+1);
  },5000);
}
function tcHommagesUpdatePlayButton(){
  var btn=document.getElementById('hommages-playpause');
  if(!btn)return;
  btn.innerHTML=HOMMAGES_PLAYING?'&#10073;&#10073;':'&#9654;';
  btn.setAttribute('aria-label',t(HOMMAGES_PLAYING?'hommages_pause':'hommages_play'));
}
function tcHommagesTogglePlay(){
  HOMMAGES_PLAYING=!HOMMAGES_PLAYING;
  if(HOMMAGES_PLAYING)tcHommagesStartAutoplay();else tcHommagesStopAutoplay();
  tcHommagesUpdatePlayButton();
}
...
function renderHommages(){
  ...
  container.innerHTML=yearBarHtml
    ...
    +'<div class="hommages-meta">'
      +'<button type="button" class="hommages-playpause" id="hommages-playpause" data-action="hommages-toggle-play"></button>'
      +'<span class="hommages-counter" id="hommages-counter"></span>'
    +'</div>';
  tcHommagesWireYearSelect();
  tcHommagesRenderCurrent();
  HOMMAGES_PLAYING=true;
  tcHommagesUpdatePlayButton();
  tcHommagesStartAutoplay();
  container.onmouseenter=function(){tcHommagesStopAutoplay();};
  container.onmouseleave=function(){if(HOMMAGES_PLAYING)tcHommagesStartAutoplay();};
...
// ── Bannière page Index — nouvel hommage de l'année en cours ─────
// Mémorise (localStorage) la signature du dernier hommage vu pour ne
// réafficher la bannière que lorsqu'un nouveau cinéaste vient s'y ajouter.
function tcHommagesBannerSignature(slides,year){
  return year+':'+slides.map(function(s){return s.cineaste.nom;}).sort().join('|');
}
function tcHommagesBannerCheck(){
  var banner=document.getElementById('hommages-banner');
  if(!banner||!DATA)return;
  var year=new Date().getFullYear();
  var slides=tcHommagesComputeSlides(year);
  if(!slides.length){banner.style.display='none';return;}
  var sig=tcHommagesBannerSignature(slides,year);
  var dismissed=null;
  try{dismissed=localStorage.getItem('tc-hommages-banner-dismissed');}catch(e){}
  if(dismissed===sig){banner.style.display='none';return;}
  var textEl=document.getElementById('hommages-banner-text');
  if(textEl)textEl.textContent=t('hommages_banner_msg',[slides.length,year]);
  banner.setAttribute('data-sig',sig);
  banner.style.display='flex';
}
function tcHommagesBannerDismiss(){
  var banner=document.getElementById('hommages-banner');
  if(!banner)return;
  var sig=banner.getAttribute('data-sig')||'';
  try{localStorage.setItem('tc-hommages-banner-dismissed',sig);}catch(e){}
  banner.style.display='none';
}
function tcHommagesBannerOpen(){
  ACTU_TAB_SELECTED='hommages';
  navigate('actualites');
}
...
function openCourantDetail(courantId){
  ...
  document.getElementById('fiche-content').innerHTML=
    '<div class="fiche-header courant-fiche-header" style="--sq:'+sqW+'">'
...
function renderStatistiques(){
  ...
  courKeys.forEach(function(k,i){
    ...
    var courYearsHtml=courYears?'<span class="ranking-years">'+courYears+'</span>':'';
    row.innerHTML='<div class="'+rankClass+'">'+(i+1)+'</div>'
      +'<div class="ranking-name">'+escapeHtml(tcCourantLabel(Number(k)))+courFlag+courYearsHtml+'</div>'
      +'<div class="ranking-value">'+nb+' '+t('occurrences_in_ranking')+'</div>';
...
document.addEventListener('click',function(e){
  ...
  switch(action){
    ...
    case 'hommages-goto':
      ...
      break;
    case 'hommages-toggle-play':
      tcHommagesTogglePlay();
      break;
    case 'hommages-banner-open':
      tcHommagesBannerOpen();
      break;
    case 'hommages-banner-close':
      tcHommagesBannerDismiss();
      break;
  }
});
