...
function renderStatistiques(){
  ...
  var ranked=shuffled
    ...
    .filter(function(c){return(cinTopCount[c.nom]||0)>=17;});
  ...
  sbAuth.from('contributors').select('id,json_name,display_name,cineaste_coeur,cineaste_autres,film_coeur,film_autres,presentation,avatar_url,is_admin').then(function(res){
    if(res.data && res.data.length){
      ...
      tcLoadAllApprovedSubmissions('*, contributors(display_name)').then(function(res2){
        ...
        // NB : PostgREST renvoie ses erreurs dans une promesse RÉSOLUE ({data:null,error:{…}}),
        // pas dans un rejet. Une requête en échec (colonne renommée/supprimée, RLS…) arrive donc
        // ici comme un simple « pas de données » et était jusqu'ici avalée silencieusement.
        if(res2&&res2.error)console.error('Erreur de la requête submissions :',res2.error);
        (res2&&res2.data?res2.data:[]).forEach(function(s){
        ...
        function loadAllTops(offset, pageSize){
          return TC_SB.from('tops').select('contributor_id, cineaste_nom, films')
            ...
            .range(offset, offset + pageSize - 1)
            .then(function(res3){
              if(res3&&res3.error){console.error('Erreur de la requête tops :',res3.error);return;}
              var rows = res3.data || [];
