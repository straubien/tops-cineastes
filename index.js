...
window.addEventListener('popstate',function(e){
  ...
  } else if(hash==='#actualites'||hash==='#contributeurs'||hash==='#statistiques'||hash==='#mes-tops'||hash==='#thematiques'){
...
function renderStatistiques(){
  ...
  // Bornes globales (début du plus ancien courant / fin du plus récent, un
  // courant sans année de fin étant considéré actif jusqu'à aujourd'hui) pour
  // positionner proportionnellement la barre de durée de chaque courant.
  var nowYear=new Date().getFullYear();
  var tlDebuts=[],tlFins=[];
  courKeys.forEach(function(k){
    var row=_TC_COURANTS_CACHE&&_TC_COURANTS_CACHE[k];
    if(!row||!row.annee_debut)return;
    tlDebuts.push(row.annee_debut);
    tlFins.push(row.annee_fin||nowYear);
  });
  var tlMin=tlDebuts.length?Math.min.apply(null,tlDebuts):null;
  var tlMax=tlFins.length?Math.max.apply(null,tlFins):null;
  var tlSpan=(tlMin!==null&&tlMax!==null)?Math.max(tlMax-tlMin,1):1;
  var lastDecade=null;
  courKeys.forEach(function(k){
    var row=_TC_COURANTS_CACHE&&_TC_COURANTS_CACHE[k];
    var debut=row&&row.annee_debut?row.annee_debut:null;
    // Repère de décennie : une ligne de séparation légère apparaît dès que le
    // courant suivant démarre dans une nouvelle décennie.
    if(debut!==null){
      var decade=Math.floor(debut/10)*10;
      if(decade!==lastDecade){
        lastDecade=decade;
        var deco=document.createElement('div');
        deco.className='courant-tl-decade';
        deco.innerHTML='<span>'+t('stat_courants_decennie',decade)+'</span>';
        courRankEl.appendChild(deco);
      }
    }
    var row_=document.createElement('div');
    row_.className='courant-tl-item';
    ...
    var nb=courantCount[k];
    var countHtml='<span class="courant-tl-count" title="'+nb+' '+escapeHtml(t('occurrences_in_ranking'))+'">'+nb+'</span>';
    var barHtml='';
    if(debut!==null&&tlMin!==null){
      var finB=row.annee_fin||nowYear;
      var leftPct=((debut-tlMin)/tlSpan)*100;
      var widthPct=Math.max(((finB-debut)/tlSpan)*100,1.5);
      var openEnded=!row.annee_fin;
      barHtml='<div class="courant-tl-bar"><div class="courant-tl-bar-fill'+(openEnded?' open-ended':'')+'" style="left:'+leftPct.toFixed(2)+'%;width:'+widthPct.toFixed(2)+'%"></div></div>';
    }
    row_.innerHTML='<div class="courant-tl-year">'+(debut||'')+'</div>'
      +'<div class="courant-tl-dot"></div>'
      +'<div class="courant-tl-content">'
        +'<div class="courant-tl-name">'+escapeHtml(tcCourantLabel(Number(k)))+courFlag+countHtml+'</div>'
        +courYearsHtml
        +barHtml
      +'</div>';
    row_.setAttribute('data-courant',k);
    row_.onclick=function(){openCourantDetail(Number(this.getAttribute('data-courant')));};
    courRankEl.appendChild(row_);
  });
