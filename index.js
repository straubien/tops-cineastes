...
function renderActualites(){
  ...
  Promise.all([
    tcWithRetryTimeout(function(){ return TC_SB.from('cineaste_proposals').select('*').eq('status','pending').order('submitted_at',{ascending:false}).limit(20); }).then(function(r){return r.data||[];}).catch(function(){return[];}),
    tcWithRetryTimeout(function(){ return TC_SB.from('submissions').select('*, contributors(display_name)').eq('status','approved').order('submitted_at',{ascending:true}); }).then(function(r){return r.data||[];}).catch(function(){return[];}),
