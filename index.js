...
function renderStatistiques(){
  ...
  // Classement des courants : on limite le nombre de lignes affichées pour
  // que la colonne se termine au même niveau que « Films dans les tops »
  // (colonne de droite), en tenant compte des lignes déjà prises par le
  // classement des cinéastes ci-dessus.
  var filmsRankCount=Object.keys(IMPORTED_COUNTS).filter(function(n){return IMPORTED_COUNTS[n].films>0;}).length;
  var courantsShowCount=Math.max(0,Math.min(courKeys.length,filmsRankCount-ranked.length));

  var courRankEl=document.getElementById('ranking-courants');
  courRankEl.innerHTML='';
  courKeys.slice(0,courantsShowCount).forEach(function(k,i){
