...
// Poids d'un point commun selon le statut favori/autre chez chacun des deux
// cinéphiles : un coup de cœur partagé pèse plus qu'une simple appréciation
// commune, elle-même un peu plus qu'un point commun "favori d'un seul côté".
function compareWeight(favA,favB){
...
function compareTwoIndexes(idxA,idxB){
  var commons=[],score=0,commonOwnWeightA=0,commonOwnWeightB=0;
  Object.keys(idxA).forEach(function(k){
    var b=idxB[k];
    if(!b)return;
    var a=idxA[k];
    var w=compareWeight(a.fav,b.fav);
    score+=w;
    commonOwnWeightA+=compareOwnWeight(a.fav);
    commonOwnWeightB+=compareOwnWeight(b.fav);
    commons.push({display:a.display,favA:a.fav,favB:b.fav,weight:w});
  });
