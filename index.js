...
var statsFilmsMode='films'; // 'films' | 'ratio' — filtre du classement "Films dans les tops"
...
var TC_SB=supabase.createClient(TC_SUPABASE_URL,TC_SUPABASE_KEY,{auth:{flowType:'implicit',detectSessionInUrl:false}});
...
function loadData(){
  ...
  init();
    ...
    var listEl=document.getElementById('cineaste-list');
    if(listEl)listEl.innerHTML='<div class="empty-msg">Erreur de chargement. <button class="btn-secondary" style="font-size:12px;padding:4px 12px;margin-left:8px" onclick="loadData()">Réessayer</button></div>';
  });
}
loadData();
...
function renderProfilFavs(favs){
  ...
  favs.forEach(function(f,fi){
    ...
    item.innerHTML='<img class="'+imgClass+'" src="'+escapeHtml(p.src)+'" alt="'+escapeHtml(f)+'" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\'" loading="lazy">'
...
function showImportedPanel(name){
  ...
  var html='<div class="fiche-header">'
    +'<button class="imp-panel-back" onclick="closeFiche()" title="Retour">&#8592;</button>'
    ...
    +'<button id="imp-sort-btn" class="btn-secondary" style="font-size:13px;padding:6px 16px;color:#000;border-color:#000" onclick="impPanelToggleSort()">↓ Films</button>'
    +'</div>'
  ...
  _renderImpPanel();
  applyLang(localStorage.getItem('tc-lang')||'fr');
}
...
function _renderImpPanel(){
  ...
  var rows=sorted.map(function(it,i){
    ...
    var editHtml=(_impPanelOwnerName===currentUserJsonName)?('<button class="btn-secondary imp-panel-edit-btn" onclick="event.stopPropagation();tcEditTop(\''+it.cineaste.replace(/'/g,"\\'")+'\')">'+window.t('mt_fiche_btn_edit')+'</button>'):'';
    ...
    return'<div class="imp-panel-block'+(hasFilms?' has-films':'')+'" id="'+blockId+'"'+(hasFilms?' onclick="toggleAccordeon(\''+blockId+'\')"':'')+'>'+rowHtml+filmsHtml+'</div>';
  }).join('');
...
function updateCurrentUser(){
  var dn=localStorage.getItem('tc-display-name');
...
function openFiche(c){
  ...
  document.getElementById('fiche-content').innerHTML=
    '<div class="fiche-header">'
      +'<button class="profil-header-back" onclick="closeFiche()" title="'+t('retour_contributeurs_title')+'">&#8592;</button>'
    ...
    +'<div class="fiche-body">'
      ...
      +'</div>'
      +(currentUserJsonName?('<button class="btn-primary fiche-submit-top-btn" onclick="tcSubmitTopFor(\''+c.nom.replace(/'/g,"\\'")+'\')">'+t('mt_fiche_btn_submit')+'</button>'):'')
    +'</div>'
    +fbHtml;
...
function tcAuthorAvatarHtml(id){
  ...
  if(url)return '<span class="tc-cm-avatar"><img src="'+url+'" alt=""></span>';
...
function tcActuAvatarHtml(id,fallbackName){
  ...
  if(url)return '<span class="actu-avatar"><img src="'+url+'" alt=""></span>';
...
function tcBubbleIconSvg(){
  return '<svg class="tc-cm-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="M21 11.5c0 4.7-4.03 8.5-9 8.5-1.06 0-2.07-.16-3-.46L4 21l1.06-3.94C4.4 15.86 4 13.74 4 11.5 4 6.8 8.03 3 13 3s8 3.8 8 8.5Z" stroke-linecap="round" stroke-linejoin="round"/></svg>';
}

function tcCommentsToggleHtml(uid,ownerId,cineasteNom){
  return '<button type="button" class="tc-cm-toggle-btn" title="'+t('tc_cm_voir')+'" aria-label="'+t('tc_cm_voir')+'" onclick="event.stopPropagation();tcToggleComments(\''+uid+'\',\''+ownerId+'\',\''+cineasteNom.replace(/'/g,"\\'")+'\')" id="'+uid+'-btn">'+tcBubbleIconSvg()+'</button>'
...
function tcCommentRowHtml(uid,cm,isReply,parentCm){
  ...
  if(currentUserContribId&&!isReply){
    actions+='<button type="button" class="tc-cm-action" onclick="tcStartReply(\''+uid+'\','+cm.id+')">'+t('tc_cm_repondre')+'</button>';
  }
  if(isOwn){
    actions+='<button type="button" class="tc-cm-action" onclick="tcStartEdit(\''+uid+'\','+cm.id+')">'+t('tc_cm_modifier')+'</button>';
  }
  if(canDelete){
    actions+='<button type="button" class="tc-cm-action tc-cm-action-danger" onclick="tcDeleteComment(\''+uid+'\','+cm.id+')">'+t('tc_cm_supprimer')+'</button>';
  }
...
function tcToolbarHtml(taId,uid){
  return '<div class="tc-cm-toolbar">'
    +'<button type="button" title="'+t('tc_cm_gras')+'" onclick="tcInsertFmt(\''+taId+'\',\'**\',\'**\')"><b>G</b></button>'
    +'<button type="button" title="'+t('tc_cm_italique')+'" onclick="tcInsertFmt(\''+taId+'\',\'*\',\'*\')"><i>I</i></button>'
    +'<button type="button" title="'+t('tc_cm_souligne')+'" onclick="tcInsertFmt(\''+taId+'\',\'__\',\'__\')"><u>S</u></button>'
    +'<button type="button" title="'+t('tc_cm_emoji')+'" onclick="tcToggleEmojiPanel(\''+uid+'\')">🙂</button>'
    +'</div>'
    +'<div class="tc-cm-emoji-panel" id="'+uid+'-emoji" style="display:none">'
      +TC_EMOJIS.map(function(em){return '<button type="button" onclick="tcInsertEmoji(\''+taId+'\',\''+em+'\')">'+em+'</button>';}).join('')
    +'</div>';
}
...
function tcComposerHtml(uid){
  ...
  var quoteHtml='';
  if(st&&st.replyTo){
    quoteHtml='<div class="tc-cm-replying-to"><span>'+t('tc_cm_en_reponse_a')+' <b>'+escapeHtml(st.replyTo.author)+'</b> : « '+escapeHtml(st.replyTo.snippet)+' »</span><button type="button" onclick="tcCancelReply(\''+uid+'\')">✕</button></div>';
  }
  return '<div class="tc-cm-composer">'
    ...
    +'<div class="tc-cm-composer-actions"><button type="button" class="btn-primary" style="padding:8px 18px;font-size:13px" onclick="tcSubmitComment(\''+uid+'\')">'+t('tc_cm_publier')+'</button></div>'
  +'</div>';
}
...
function tcEditFormHtml(uid,cm){
  ...
  return '<div class="tc-cm-main tc-cm-edit-form">'
    ...
    +'<div class="tc-cm-composer-actions">'
      +'<button type="button" class="btn-secondary" style="padding:6px 14px;font-size:13px" onclick="tcCancelEdit(\''+uid+'\')">'+t('tc_cm_annuler')+'</button>'
      +'<button type="button" class="btn-primary" style="padding:6px 14px;font-size:13px" onclick="tcSaveEdit(\''+uid+'\','+cm.id+')">'+t('tc_cm_enregistrer')+'</button>'
    +'</div>'
  +'</div>';
}
...
function tcSubmitComment(uid){
  ...
  ta.disabled=true;
  TC_SB.from('comments').insert(payload).select('*').single().then(function(res){
    ta.disabled=false;
...
function tcSaveEdit(uid,commentId){
  ...
  var body=ta.value.trim();
  if(!body)return;
  TC_SB.from('comments').update({body:body,updated_at:new Date().toISOString()}).eq('id',commentId).select('*').single().then(function(res){
    if(res.error){alert(t('tc_cm_erreur_envoi'));return;}
...
function tcDeleteComment(uid,commentId){
  ...
  TC_SB.from('comments').delete().eq('id',commentId).select('id').then(function(res){
    if(res.error){alert(t('tc_cm_erreur_envoi'));return;}
...
function tcActuOpenCineaste(nom){
  if(!DATA)return;
  var c=DATA.cineastes.find(function(x){return x.nom.toLowerCase()===nom.toLowerCase();});
  if(c)openFiche(c);
}
...
function renderActualites(){
  ...
  Promise.all([
    TC_SB.from('cineaste_proposals').select('*').eq('status','pending').order('submitted_at',{ascending:false}).limit(20).then(function(r){return r.data||[];}).catch(function(){return[];}),
    TC_SB.from('submissions').select('*, contributors(display_name)').eq('status','approved').order('submitted_at',{ascending:true}).then(function(r){return r.data||[];}).catch(function(){return[];}),
    TC_SB.from('comments').select('*').is('parent_comment_id',null).order('created_at',{ascending:false}).limit(20).then(function(r){return r.data||[];}).catch(function(){return[];}),
    ...
    props.forEach(function(p){
      ...
      items.push({date:p.submitted_at,html:'<span class="actu-icon">🎬</span> '+t('actu_proposition',escapeHtml(nomComplet))});
    });
    ...
    subs.forEach(function(s){
      ...
      items.push({date:s.submitted_at,html:tcActuAvatarHtml(s.contributor_id,dn)+' '+label,cineaste:cin});
    });
    ...
    feed.innerHTML=items.map(function(it){
      ...
      var attr=clickable?' onclick="tcActuOpenCineaste(\''+it.cineaste.replace(/'/g,"\\'")+'\')"':'';
...
function renderStatistiques(){
  ...
  ranked.forEach(function(c,i){
    ...
    row.onclick=function(){
      var fbid=this.getAttribute('data-fbid');
      navigate('index');
      setTimeout(function(){var x=DATA.cineastes.find(function(y){return y.fbid===fbid;});if(x)openFiche(x);},100);
    };
...
function renderFilmsRanking(){
  var isRatio=statsFilmsMode==='ratio';
  var filmsRanking=[];
  Object.keys(IMPORTED_COUNTS).forEach(function(name){
    var imp=IMPORTED_COUNTS[name];
    if(imp.films>0&&imp.tops>0)filmsRanking.push({name:name,films:imp.films,ratio:imp.films/imp.tops});
  });
  filmsRanking.sort(function(a,b){return isRatio?(b.ratio-a.ratio):(b.films-a.films);});
  ...
  if(!filmsRanking.length){
    ...
    filmsRanking.forEach(function(c,i){
      ...
      var valueHtml=isRatio?(c.ratio.toFixed(1)+' '+t('ratio_films_top_in_ranking')):(c.films+' '+t('films_in_ranking'));
      row.innerHTML='<div class="'+rankClass+'">'+(i+1)+'</div>'
        +'<div class="ranking-name">'+formatContribNamePlain(c.name)+'</div>'
        +'<div class="ranking-value">'+valueHtml+'</div>';
...
document.getElementById('ranking-films-toggle').querySelectorAll('.ranking-toggle-btn').forEach(function(btn){
  btn.addEventListener('click',function(){
    if(this.classList.contains('active'))return;
    document.getElementById('ranking-films-toggle').querySelectorAll('.ranking-toggle-btn').forEach(function(b){b.classList.remove('active');});
    this.classList.add('active');
    statsFilmsMode=this.getAttribute('data-mode');
    renderFilmsRanking();
  });
});
...
(function(){
  ...
  sbAuth.from('contributors').select('id,json_name,display_name,cineaste_coeur,cineaste_autres,film_coeur,film_autres,presentation,avatar_url,is_admin').then(function(res){
    if(res.data && res.data.length){
      ...
      res.data.forEach(function(c){
        ...
        if(el){
          ...
          img.onload=function(){
            ...
            if(cur){var imgEl=document.createElement('img');imgEl.className='contrib-avatar';imgEl.src=c.avatar_url;imgEl.alt=c.json_name;cur.parentNode.replaceChild(imgEl,cur);}
          };
      ...
      sbAuth.from('submissions').select('*, contributors(display_name)').eq('status','approved').then(function(res2){
  ...
  function buildNavAvatarHtml(jsonName, displayName){
    ...
    if(url) return '<span class="nav-user-avatar"><img src="'+url+'" alt=""></span>';
  ...
  sbAuth.auth.onAuthStateChange(function(event, session){
    if(event === 'INITIAL_SESSION'){
      if(session){
        var displayName = localStorage.getItem('tc-display-name');
  ...
  document.getElementById('nav-logout-tip').addEventListener('click', function(){
    localStorage.removeItem('tc-display-name');
    sbAuth.auth.signOut();
  });
...
(function(){
  ...
  async function mtCheckNotifications(){
    if(!mtCurrentContributor) return;
    await mtFlushPendingSeen();
    var subRes = await sbMT.from('submissions').select('id, status, parsed_json')
      .eq('contributor_id', mtCurrentContributor.id)
      .in('status', ['approved', 'rejected'])
      .is('seen_at', null);
    var propRes = await sbMT.from('cineaste_proposals').select('id, nom, prenom')
      .eq('contributor_id', mtCurrentContributor.id)
      .eq('status', 'approved')
      .is('seen_at', null);

    var myOwnComments = await sbMT.from('comments').select('id').eq('author_contributor_id', mtCurrentContributor.id);
    var myOwnIds = (myOwnComments.data || []).map(function(r){ return r.id; });
    var commentNotifs = [];
    var topLevelRes = await sbMT.from('comments').select('id, author_contributor_id, cineaste_nom')
      .eq('top_contributor_id', mtCurrentContributor.id)
      .is('parent_comment_id', null)
      .neq('author_contributor_id', mtCurrentContributor.id)
      .is('seen_at', null);
    commentNotifs = commentNotifs.concat(topLevelRes.data || []);
    if(myOwnIds.length){
      var repliesRes = await sbMT.from('comments').select('id, author_contributor_id, cineaste_nom')
        .in('parent_comment_id', myOwnIds)
        .neq('author_contributor_id', mtCurrentContributor.id)
        .is('seen_at', null);
      commentNotifs = commentNotifs.concat(repliesRes.data || []);
    }
    ...
    (subRes.data || []).forEach(function(s){
      ...
      mtUnseenNotifications.push({
        ...
        text: s.status === 'approved'
          ? ('Votre top pour ' + cineaste + ' a été validé.')
          : ('Votre top pour ' + cineaste + ' a été refusé.')
      });
    });
    (propRes.data || []).forEach(function(p){
      ...
      mtUnseenNotifications.push({
        ...
        text: 'Votre suggestion de cinéaste "' + nomComplet + '" a été validée.'
      });
    });
    commentNotifs.forEach(function(cm){
      ...
      mtUnseenNotifications.push({
        ...
        text: tcAuthorLabel(cm.author_contributor_id) + ' a commenté le top "' + cm.cineaste_nom + '".'
      });
    });
  ...
  async function mtFlushPendingSeen(){
    ...
    for(var i=0;i<list.length;i++){
      var p = list[i];
      var res = await sbMT.from(mtSeenTableFor(p.kind)).update({ seen_at: now }).eq('id', p.id).select('id');
      if(res.error){
  ...
  async function mtMarkNotificationsSeen(){
    if(!mtUnseenNotifications.length) return;
    mtUnseenNotifications.forEach(function(n){ mtAddDismissedId(n.kind, n.id); });
    var subIds = mtUnseenNotifications.filter(function(n){ return n.kind === 'submission'; }).map(function(n){ return n.id; });
    var propIds = mtUnseenNotifications.filter(function(n){ return n.kind === 'proposal'; }).map(function(n){ return n.id; });
    var commentIds = mtUnseenNotifications.filter(function(n){ return n.kind === 'comment'; }).map(function(n){ return n.id; });
    var now = new Date().toISOString();
    if(subIds.length){
      var subRes2 = await sbMT.from('submissions').update({ seen_at: now }).in('id', subIds).select('id');
      if(subRes2.error){ console.error('Erreur marquage seen_at submissions:', subRes2.error.message); subIds.forEach(function(id){ mtQueuePendingSeen('submission', id); }); }
      else if(!subRes2.data || !subRes2.data.length){ subIds.forEach(function(id){ mtQueuePendingSeen('submission', id); }); }
    }
    if(propIds.length){
      var propRes2 = await sbMT.from('cineaste_proposals').update({ seen_at: now }).in('id', propIds).select('id');
      if(propRes2.error){ console.error('Erreur marquage seen_at cineaste_proposals:', propRes2.error.message); propIds.forEach(function(id){ mtQueuePendingSeen('proposal', id); }); }
      else if(!propRes2.data || !propRes2.data.length){ propIds.forEach(function(id){ mtQueuePendingSeen('proposal', id); }); }
    }
    if(commentIds.length){
      var comRes2 = await sbMT.from('comments').update({ seen_at: now }).in('id', commentIds).select('id');
      if(comRes2.error){ console.error('Erreur marquage seen_at comments:', comRes2.error.message); commentIds.forEach(function(id){ mtQueuePendingSeen('comment', id); }); }
      else if(!comRes2.data || !comRes2.data.length){ commentIds.forEach(function(id){ mtQueuePendingSeen('comment', id); }); }
    }
    mtUnseenNotifications = [];
    var navMesTops = document.getElementById('nav-mes-tops');
    var navSoumettre = document.getElementById('nav-soumettre');
    if(navMesTops) navMesTops.classList.remove('has-notif');
  ...
  window.mtOnNavigate = function(){
    ...
    return mtLoadPromise.then(function(){
      ...
      mtRenderNotifBanner();
      return mtMarkNotificationsSeen();
    });
  };
  ...
  var _mtAc = createAutocomplete({
    ...
    onSelect: function(nom){ mtSelectedCineaste = nom; }
  });

  function mtCheckDuplicate(cineaste){
    if(mtEditBypassCineaste && normStr(mtEditBypassCineaste) === normStr(cineaste)) return false;
    if(mtCurrentContributor && SUPABASE_TOPS[mtCurrentContributor.json_name] && SUPABASE_TOPS[mtCurrentContributor.json_name][cineaste]) return true;
  ...
  function mtParseTops(){
    ...
    if(mtCheckDuplicate(cineaste)){
      if(dupMsg){ dupMsg.textContent = t('mt_duplicate_msg'); dupMsg.style.display = 'block'; }
      return;
    } else if(dupMsg){ dupMsg.style.display = 'none'; }
  ...
  async function mtSubmitTops(){
    ...
    var res = await sbMT.from('submissions').insert({
  ...
  async function mtLoadPrevSubmissions(){
    if(!mtCurrentContributor) return;
    var res = await sbMT.from('submissions').select('id, parsed_json, status, submitted_at').eq('contributor_id', mtCurrentContributor.id).order('submitted_at', { ascending: false }).limit(50);

    if(res.error || !res.data || !res.data.length){ mtLastSubmissions = []; document.getElementById('mt-prev-section').style.display = 'none'; return; }
  ...
  async function mtLoadProposals(){
    ...
    var res = await sbMT.from('cineaste_proposals')
  ...
  async function mtToggleVote(proposalId, currentlyVoted){
    if(currentlyVoted){
      var res = await sbMT.from('cineaste_proposal_votes')
        .delete()
      ...
      var res = await sbMT.from('cineaste_proposal_votes').insert({
  ...
  document.getElementById('prop-btn-submit').addEventListener('click', async function(){
    ...
    var res = await sbMT.from('cineaste_proposals').insert({
      contributor_id: mtCurrentContributor.id,
