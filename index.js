...
function tcTouchLastSeen(){
  ...
  TC_SB.from('contributors').update({last_seen_at:iso}).eq('id',currentUserContribId)
    .then(function(res){
      if(res&&res.error)return;
      TC_LAST_SEEN_LOCAL[String(currentUserContribId)]=Date.now();
      var me=CONTRIB_DATA.find(function(c){return String(c.id)===String(currentUserContribId);});
      if(me)me.last_seen_at=iso;
    },function(){}); // erreur réseau : on ignore, le prochain tick réessaiera
}
...
(function(){
  ...
  sbAuth.from('contributors').select('id,json_name,display_name,cineaste_coeur,cineaste_autres,film_coeur,film_autres,presentation,avatar_url,is_admin').then(function(res){
    if(res.data && res.data.length){
      ...
      if(currentUserContribId)tcStartLastSeenHeartbeat();
      tcRenderIndexPresence();
      tcReloadLastSeen();
