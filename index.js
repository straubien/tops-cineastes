...
function renderProfilPortrait(name,cin){
  ...
  cin.forEach(function(c){
    // Un duo stocke « vivant » sous forme de tableau [statutA, statutB] : on
    // compte alors chaque membre séparément (sinon les duos étaient ignorés).
    var statuts=Array.isArray(c.vivant)?c.vivant:[c.vivant];
    statuts.forEach(function(v){
      if(v===true)enActivite++;
      else if(v===false)disparus++;
    });
  ...
  // 6) Synthèse — profil en une phrase (assemblée à partir des données réelles)
  var parts=[];
  if(topDecade!==null)parts.push(t('pp_synth_epoque',topDecade));
  if(topPays)parts.push(t('pp_synth_cinema',tcCountryLabel(topPays)||topPays));
  if(pctActif!==null)parts.push(t('pp_synth_actifs',pctActif));
  if(parts.length){
    html+='<div class="profil-presentation pp-synth">'
      +'<div class="profil-presentation-label">'+t('pp_synthese')+'</div>'
      +'<div class="pp-synth-text">'+parts.join(' &nbsp;·&nbsp; ')+'.</div></div>';
  }
