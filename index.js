...
async function renderThematiques(){
  ...
  themes.forEach(function(nom){
    ...
    var photoHtml = photo
      ? '<img class="cineaste-photo" src="' + escapeHtml(photo) + '" alt="" loading="lazy">'
      : '<div class="cineaste-photo cineaste-photo-empty"></div>';

    row.innerHTML = photoHtml
      + '<div class="cineaste-info">'
      + '<div class="cineaste-nom"><b>' + escapeHtml(nom) + '</b></div>'
      + '</div>'
      + '<div class="cineaste-badge">' + nbContribs + '</div>';
...
function openFicheThematique(themeNom){
  ...
  var photoHtml = photo
    ? '<img style="width:80px;height:80px;object-fit:cover;margin-bottom:1rem;" src="' + escapeHtml(photo) + '" alt="">'
    : '';

  var contribsHtml = rows.map(function(row){
    ...
    var filmsHtml = '<ol class="fiche-films-list" style="padding-left:1.5rem;margin:0.4rem 0 1rem">'
      + films.map(function(f){
          var titre = typeof f === 'string' ? f : (f.titre || '');
          var annee = f.annee ? ' <span style="color:var(--brun-pale);font-size:0.85em">(' + escapeHtml(String(f.annee)) + ')</span>' : '';
          return '<li style="margin-bottom:4px">' + escapeHtml(titre) + annee + '</li>';
        }).join('')
      + '</ol>';
    return '<div style="margin-bottom:1.4rem">'
      + '<div style="font-size:17px;font-weight:700;margin-bottom:4px">' + escapeHtml(name) + '</div>'
      + filmsHtml
      + '</div>';
  }).join('');

  content.innerHTML = '<button data-action="close-fiche" class="fiche-close" aria-label="Fermer">×</button>'
    + photoHtml
    + '<h2 style="font-size:28px;font-weight:700;margin-bottom:1.4rem">' + escapeHtml(themeNom) + '</h2>'
    + contribsHtml;
