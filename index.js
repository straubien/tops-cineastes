...
var PHOTOS_THEMES = {}; // theme_nom -> chemin image (photos-themes.json)
...
function loadData(){
  ...
  Promise.all([
    ...
    tcFetchWithTimeout('photos-themes.json').then(function(r){return r.json()}).catch(function(){return null;})
  ]).then(function(results){
  var cineastes=results[0]||[];
  var muzard=results[1];
  var cnudde=results[2];
  PHOTOS_TMDB=results[3]||{};
  PHOTOS_THEMES=results[4]||{};
...
async function renderThematiques(){
  ...
  themes.forEach(function(nom){
    ...
    var photo = PHOTOS_THEMES[nom];

    var row = document.createElement('div');
    row.className = 'cineaste-row';

    var photoHtml = photo
      ? '<img class="c-photo" src="' + escapeHtml(photo) + '" alt="" loading="lazy">'
      : '<div class="c-photo c-photo-empty"></div>';

    row.innerHTML = '<div class="c-bar"></div>'
      + photoHtml
      + '<div class="c-info"><span class="c-name"><b>' + escapeHtml(nom) + '</b></span></div>'
      + '<div class="c-badge">' + nbContribs + '</div>';
...
function openFicheThematique(themeNom){
  ...
  var nb = rows.length;
  var photo = PHOTOS_THEMES[themeNom];

  var fichePhotoHtml = photo
    ? '<img class="fiche-photo" src="' + escapeHtml(photo) + '" alt="" loading="lazy">'
    : '<div class="fiche-photo fiche-photo-empty"></div>';
