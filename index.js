...
function tcCommentsToggleHtml(uid,ownerId,cineasteNom){
  return '<button type="button" class="tc-cm-toggle-btn" onclick="event.stopPropagation();tcToggleComments(\''+uid+'\',\''+ownerId+'\',\''+cineasteNom.replace(/'/g,"\\'")+'\')" id="'+uid+'-btn">💬 '+t('tc_cm_voir')+'</button>'
...
function tcRenderComments(uid){
  ...
  if(btn)btn.innerHTML='💬 '+t('tc_cm_voir')+(st.rows.length?' ('+st.rows.length+')':'');
...
function renderActualites(){
  ...
  Promise.all([
    ...
    subs.forEach(function(s){
      ...
      var isModif=(s.parsed_json&&s.parsed_json.approvalCount>1)||(key&&(legacyKeys.has(key)||seenKeys.has(key)));
      if(key)seenKeys.add(key);
      var label=isModif?t('actu_top_modifie',[escapeHtml(dn),formatNom(cin)]):t('actu_top_nouveau',[escapeHtml(dn),formatNom(cin)]);
      items.push({date:s.submitted_at,html:'<span class="actu-icon">✅</span> '+label});
    });
    coms.forEach(function(c){
      var dn=tcAuthorLabel(c.author_contributor_id);
      items.push({date:c.created_at,html:'<span class="actu-icon">💬</span> '+t('actu_commentaire',[escapeHtml(dn),formatNom(c.cineaste_nom||'')])});
    });
    ...
    feed.innerHTML=items.map(function(it){
      return'<div class="actu-item"><div class="actu-text">'+it.html+'</div><div class="actu-date">'+tcActuFormatDate(it.date)+'</div></div>';
    }).join('');
  }).catch(function(){
...
function renderContributeurs(){
  ...
  _contribs.forEach(function(c){
    ...
    var avatarHtml='<div class="contrib-avatar-placeholder" id="'+avId+'">'+initiales+'</div>';
