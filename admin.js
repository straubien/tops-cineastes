...
async function renderStats(){
  ...
  var courantsChartEl = document.getElementById('stats-chart-courants');
  var categoriesChartEl = document.getElementById('stats-chart-categories');
  if(!courantsChartEl || !categoriesChartEl) return;
  if(errEl) errEl.style.display = 'none';
  ...
  // Type de chaque entrée du catalogue : sert à ventiler une affectation
  // dans la section "courants" ou dans la section "catégories".
  var typeById = {};
  rows.forEach(function(row){ typeById[row.id] = row.type === 'categorie' ? 'categorie' : 'courant'; });

  // Par section, deux totaux distincts car une fiche peut porter jusqu'à trois
  // courants : "fiches" = cinéastes DISTINCTS rattachés à au moins une entrée
  // de la section ; "affectations" = nombre de cases courant/courant2/courant3
  // remplies. Les deux sont affichés dans le bandeau de synthèse.
  var countById = {};
  var totals = {
    courant:   { fiches: 0, affectations: 0 },
    categorie: { fiches: 0, affectations: 0 }
  };
  cineastes.forEach(function(c){
    ...
    [c.courant, c.courant2, c.courant3].forEach(function(id){
      ...
      totals[type].affectations++;
      vu[type] = true;
    });
    if(vu.courant) totals.courant.fiches++;
    if(vu.categorie) totals.categorie.fiches++;
  });
  ...
  rows.forEach(function(row){
    ...
    if(typeById[row.id] === 'categorie') categoriesList.push(item);
    else courantsList.push(item);
  });

  renderStatsChart(courantsChartEl, courantsList, totals.courant, cineastes.length, 'courant');
  renderStatsChart(categoriesChartEl, categoriesList, totals.categorie, cineastes.length, 'categorie');
}
...
// Rendu d'une section : bandeau de synthèse (totaux), barres horizontales
// classées par nombre décroissant de fiches, échelle puis légende de lecture.
function renderStatsChart(containerEl, items, totals, totalFiches, kind){
  ...
  var estCategorie = kind === 'categorie';
  var motSection = estCategorie ? 'catégorie' : 'courant';

  containerEl.appendChild(buildStatsSummary({
    fiches: totals.fiches,
    affectations: totals.affectations,
    totalFiches: totalFiches,
    utilises: utilises,
    catalogue: items.length,
    estCategorie: estCategorie,
    motSection: motSection
  }));
  ...
  legend.textContent = 'Longueur des barres proportionnelle au maximum de la section ('
    + statsFmtNb(maxCount) + ' fiches) ; le pourcentage indique la part des '
    + statsFmtNb(totalFiches) + ' fiches cinéaste portant ' + (estCategorie ? 'cette ' : 'ce ') + motSection + '.';
  containerEl.appendChild(legend);
}

// Bandeau supérieur : le total demandé (cinéastes rattachés à au moins une
// entrée de la section) mis en avant, puis trois indicateurs secondaires.
function buildStatsSummary(info){
  var wrap = document.createElement('div');
  ...
  mainVal.textContent = statsFmtNb(info.fiches);
  var mainLbl = document.createElement('span');
  mainLbl.className = 'stats-summary-label';
  mainLbl.textContent = (info.fiches > 1 ? 'cinéastes rattachés' : 'cinéaste rattaché')
    + ' à au moins ' + (info.estCategorie ? 'une catégorie' : 'un courant');
  ...
  [
    [info.totalFiches ? statsFmtPct(info.fiches / info.totalFiches) : '—',
     'des ' + statsFmtNb(info.totalFiches) + ' fiches'],
    [statsFmtNb(info.affectations),
     'affectations au total'],
    [statsFmtNb(info.utilises) + ' / ' + statsFmtNb(info.catalogue),
     info.estCategorie ? 'catégories utilisées' : 'courants utilisés']
  ].forEach(function(pair){
    var kpi = document.createElement('div');
    kpi.className = 'stats-summary-kpi';
...
// Une ligne du graphique : rang, drapeau du pays d'origine, nom (et période),
// barre proportionnelle, effectif et part du total.
