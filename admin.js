...
// Rafraîchit automatiquement l'onglet Statistiques (pas de flux temps réel
// Supabase Realtime : on recalcule périodiquement pendant que l'onglet est
// affiché, plus immédiatement après toute action admin qui change un
// rattachement courant/catégorie — voir refreshStatsIfActive()).
var statsRefreshInterval = null;
var isStatsTabActive = false;
function startStatsAutoRefresh(){
  if(statsRefreshInterval) clearInterval(statsRefreshInterval);
  statsRefreshInterval = setInterval(function(){
    if(currentUser && isStatsTabActive && !document.hidden) renderStats();
  }, 20000);
}

document.addEventListener('visibilitychange', function(){
  if(!document.hidden && currentUser && isDashboardTabActive) renderDashboard();
  if(!document.hidden && currentUser && isStatsTabActive) renderStats();
});
...
document.querySelectorAll('.tab').forEach(function(tab){
  tab.addEventListener('click', function(){
    ...
    isStatsTabActive = tab.getAttribute('data-tab') === 'statistiques';
...
function isTopModification(s){
  ...
  if(s.parsed_json && s.parsed_json.isModification) return true;
...
function buildProposalCard(p, container){
    ...
    if(p.status === 'pending'){
      ...
      var editRow2 = document.createElement('div'); editRow2.className = 'prop-edit-row';
      editRow2.appendChild(paysSelect); editRow2.appendChild(naissanceInput); editRow2.appendChild(decesInput);
      var editRow3 = document.createElement('div'); editRow3.className = 'prop-edit-row';
      editRow3.appendChild(photoInput);

      editZone.appendChild(editRow1); editZone.appendChild(editRow2); editZone.appendChild(editRow3);
      ...
      approveBtn.addEventListener('click', function(){
        var editFields = null;
        var confirmName = nomComplet;
        if(editZone.classList.contains('visible')){
          var editedNom = nomInput.value.trim().toUpperCase();
          if(!editedNom){ showAdminNotice('Le nom ne peut pas être vide.', false); return; }
          editFields = {
            prenom: prenomInput.value.trim(),
            nom: editedNom,
            pays: paysSelect.value || null,
            annee_naissance: naissanceInput.value ? parseInt(naissanceInput.value, 10) : null,
            annee_deces: decesInput.value ? parseInt(decesInput.value, 10) : null,
            photo_tmdb: photoInput.value.trim() || null
          };
...
async function updateProposal(p, status, approveBtn, rejectBtn, editFields){
  var id = p.id;
  approveBtn.disabled = true; rejectBtn.disabled = true;
  var updatePayload = { status: status, reviewed_at: new Date().toISOString() };
  if(editFields) Object.assign(updatePayload, editFields);
  var res;
  try {
  ...
  if(editFields) Object.assign(p, editFields);
...
// CORRECTIF BLOCAGE RECHERCHE : renderFlagsAdmin() reconstruit potentiellement
// des dizaines de lignes (2 <select> pays par cinéaste trouvé) à chaque frappe.
// On attend une courte pause dans la saisie avant de relancer le rendu, pour
// ne pas figer la page sur chaque caractère tapé.
var _flagsSearchDebounce = null;
document.getElementById('flags-search').addEventListener('input', function(e){
  ...
  clearTimeout(_flagsSearchDebounce);
  _flagsSearchDebounce = setTimeout(renderFlagsAdmin, 200);
});
...
function buildCourantCard(row){
  ...
  // Un admin peut se voir retirer, individuellement, le droit de valider les
  // courants (colonne contributors.can_validate_courants) tant que le créateur
  // du site ne le lui a pas accordé. Absence de la colonne / valeur non false
  // = comportement inchangé (autorisé), pour ne rien casser tant qu'elle n'existe
  // pas encore côté base.
  var canValidateCourants = !currentContributor || currentContributor.can_validate_courants !== false;

  var actionsHtml = '';
  if(row.status === 'pending'){
    if(canValidateCourants){
      actionsHtml = '<div class="submission-actions">'
        + '<button class="btn-approve" data-id="' + row.id + '">Valider</button>'
        + '<button class="btn-reject" data-id="' + row.id + '">Rejeter</button>'
        + '</div>';
  ...
  if(row.status === 'pending' && canValidateCourants){
    var apprBtn = body.querySelector('.btn-approve');
    var rejBtn = body.querySelector('.btn-reject');
    apprBtn.addEventListener('click', function(e){
      e.stopPropagation();
...
// Dernières lignes lues du catalogue : elles alimentent aussi les sélecteurs
...
async function loadCourantCatalogue(){
  ...
  // Le catalogue fraîchement lu sert aussi de source aux sélecteurs
  // d'emplacement et au cache partagé des libellés (_TC_COURANTS_CACHE), pour
  // qu'un renommage soit immédiatement visible partout dans la page.
  _courantCatalogueRows = res.data || [];
  var cacheMap = {};
  _courantCatalogueRows.forEach(function(row){ cacheMap[row.id] = row; });
  _TC_COURANTS_CACHE = cacheMap;
  renderCourantCatalogue(_courantCatalogueRows);
  if(_courantFicheSelected) renderCourantFicheResult(_courantFicheSelected);
}
  ...
  rows.forEach(function(row){
    ...
    saveBtn.addEventListener('click', function(){
      saveCourantEdits(row.id, {
        type: typeSel.value,
        nomFr: frInput.value.trim(),
        nomFrAvant: row.nom_fr,
    ...
    // Résumé cliquable affiché seul sur mobile (une ligne par courant) ;
    // le clic déplie le reste des champs (édition) via la classe "expanded".
    ...
    mobileNom.textContent = row.nom_fr;
    ...
    mobileHeader.appendChild(mobileFlag);
...
// Le nom français est la clé recopiée dans courant_proposals.courant et dans
// notifications.data->>'courant'. Son renommage est propagé à ces deux tables,
// côté base, par le trigger tc_courants_rename_cascade (tc-courants-cascade.sql) :
// la cascade est donc garantie quelle que soit l'origine de la modification.
async function saveCourantEdits(id, edits, btn){
  if(!edits.nomFr){ showAdminNotice('Le nom français ne peut pas être vide.', false); return; }
  ...
  var renomme = edits.nomFr !== edits.nomFrAvant;
  ...
  if(!res.data || !res.data.length){ showAdminNotice('La mise à jour n\'a pas abouti — vérifiez les droits de la table courants.', false); if(btn) btn.disabled = false; return; }
  if(btn) btn.disabled = false;
  if(renomme){
    showAdminNotice('Renommé en « ' + edits.nomFr + ' » — propositions et notifications mises à jour en cascade.', true);
...
async function addCourantToCatalogue(){
  var typeSel = document.getElementById('courant-gestion-type');
...
// ── Courants et catégories d'une fiche cinéaste ───────────────────────────
// Trois emplacements (colonnes courant / courant2 / courant3). L'admin peut y
// changer le courant affecté, le vider, ou échanger deux emplacements pour
// modifier la « place » d'un courant sans toucher à son contenu. Tout est
// appliqué en une fois par la fonction SQL tc_admin_set_courants, qui écrit la
// fiche ET propage le changement à courant_proposals et notifications dans la
// même transaction (voir tc-courants-cascade.sql).
var _courantFicheSearchInitDone = false;
var _courantFicheSelected = null;
// État d'édition local des trois emplacements (id de courant, ou null si vide).
var _courantFicheSlots = [null, null, null];
...
function initCourantFicheSearch(){
  ...
  document.getElementById('courant-fiche-cineaste-input').addEventListener('input', function(){
    ...
    _courantFicheSlots = [null, null, null];
    document.getElementById('courant-fiche-result').innerHTML = '';
  });
}

// Un id de courant normalisé en nombre (ou null) : les colonnes sont en int8,
// que le client peut renvoyer en nombre comme en chaîne selon le contexte.
function courantSlotId(v){
  return v ? Number(v) : null;
}

// Catalogue de référence pour les sélecteurs : les lignes fraîchement lues par
// l'onglet si elles existent, sinon le cache chargé au démarrage de la page.
function courantCatalogueRows(){
  if(_courantCatalogueRows && _courantCatalogueRows.length) return _courantCatalogueRows;
  var cache = _TC_COURANTS_CACHE || {};
  return Object.keys(cache).map(function(k){ return cache[k]; });
}

function courantNomFr(id){
  if(!id) return '';
  var row = courantCatalogueRows().find(function(r){ return Number(r.id) === Number(id); });
  return row ? row.nom_fr : ('#' + id);
}

// Options d'un sélecteur d'emplacement : tout le catalogue, courants et
// catégories séparés, libellés en français (la référence stockée en base).
function courantSlotOptionsHtml(selectedId){
  var rows = courantCatalogueRows().slice().sort(function(a, b){
    return String(a.nom_fr || '').localeCompare(String(b.nom_fr || ''), 'fr');
  });
  var opt = function(r){
...
// Repart de l'état enregistré en base : sert à l'ouverture d'une fiche comme au
...
function renderCourantFicheResult(nom){
  ...
  _courantFicheSlots = [courantSlotId(c.courant), courantSlotId(c.courant2), courantSlotId(c.courant3)];
  renderCourantFicheSlots(c);
}

// Rendu des trois emplacements depuis l'état local, rejoué après chaque
// échange de position pour que l'ordre affiché soit toujours celui qui sera
// enregistré.
function renderCourantFicheSlots(c){
  var resEl = document.getElementById('courant-fiche-result');
  if(!resEl) return;
  resEl.innerHTML = '';

  _courantFicheSlots.forEach(function(id, i){
    var rowEl = document.createElement('div');
    rowEl.className = 'courant-slot-row';

    var label = document.createElement('div');
    label.className = 'courant-slot-label';
    label.textContent = 'Emplacement ' + (i + 1);

    var sel = document.createElement('select');
    sel.className = 'contrib-select';
    sel.innerHTML = courantSlotOptionsHtml(id);
    sel.addEventListener('change', function(){
      _courantFicheSlots[i] = sel.value ? Number(sel.value) : null;
    });

    var move = document.createElement('div');
    move.className = 'courant-slot-move';
    var upBtn = document.createElement('button');
    upBtn.type = 'button';
    upBtn.textContent = '↑';
    upBtn.title = 'Échanger avec l\'emplacement précédent';
    upBtn.disabled = i === 0;
    upBtn.addEventListener('click', function(){ swapCourantFicheSlots(c, i, i - 1); });
    var downBtn = document.createElement('button');
    downBtn.type = 'button';
    downBtn.textContent = '↓';
    downBtn.title = 'Échanger avec l\'emplacement suivant';
    downBtn.disabled = i === _courantFicheSlots.length - 1;
    downBtn.addEventListener('click', function(){ swapCourantFicheSlots(c, i, i + 1); });
    move.appendChild(upBtn);
    move.appendChild(downBtn);

    rowEl.appendChild(label);
    rowEl.appendChild(sel);
    rowEl.appendChild(move);
    resEl.appendChild(rowEl);
  });

  var actions = document.createElement('div');
  actions.className = 'courant-slot-actions';
  var saveBtn = document.createElement('button');
  saveBtn.className = 'btn-primary';
  saveBtn.textContent = 'Enregistrer les emplacements';
  saveBtn.addEventListener('click', function(){ saveCourantFicheSlots(c, saveBtn); });
  var resetBtn = document.createElement('button');
  resetBtn.className = 'btn-reject';
  resetBtn.textContent = 'Annuler les modifications';
  resetBtn.addEventListener('click', function(){ renderCourantFicheResult(c.nom); });
  actions.appendChild(saveBtn);
  actions.appendChild(resetBtn);
  resEl.appendChild(actions);
}

// Échange deux emplacements : seule façon de changer la place d'un courant
// sans toucher au courant lui-même.
function swapCourantFicheSlots(c, i, j){
  var tmp = _courantFicheSlots[i];
  _courantFicheSlots[i] = _courantFicheSlots[j];
  _courantFicheSlots[j] = tmp;
  renderCourantFicheSlots(c);
}

// Contrôles côté navigateur (doublon, absence de changement) puis confirmation
// détaillant ce qui va être écrit. La fonction SQL revérifie de son côté : elle
// est le seul garde-fou qui compte, ceci n'évite qu'un aller-retour inutile.
function saveCourantFicheSlots(c, btn){
  var slots = _courantFicheSlots.slice();
  var remplis = slots.filter(function(v){ return v; });
  var doublon = remplis.some(function(v, idx){ return remplis.indexOf(v) !== idx; });
  if(doublon){
    showAdminNotice('Un même courant ne peut pas occuper deux emplacements.', false);
    return;
  }

  var avant = [courantSlotId(c.courant), courantSlotId(c.courant2), courantSlotId(c.courant3)];
  var inchange = slots.every(function(v, idx){ return v === avant[idx]; });
  if(inchange){
    showAdminNotice('Aucun changement à enregistrer.', false);
    return;
  }

  var ligne = function(v){ return v ? courantNomFr(v) : '(vide)'; };
  var recap = slots.map(function(v, idx){
    return (idx + 1) + '. ' + ligne(avant[idx]) + (v === avant[idx] ? '' : ' → ' + ligne(v));
  }).join('\n');

  showConfirmModal(
    'Enregistrer les emplacements de ' + c.nom + ' ?\n\n' + recap
      + '\n\nLes propositions validées et les notifications liées seront mises à jour en conséquence.',
    function(){ applyCourantFicheSlots(c, slots, btn); }
  );
}

// Écriture effective : un seul appel RPC, donc une seule transaction côté base
// pour la fiche et pour la cascade.
async function applyCourantFicheSlots(c, slots, btn){
  if(btn) btn.disabled = true;
  var res;
  try {
    res = await tcWithRetryTimeout(function(){
      return sb.rpc('tc_admin_set_courants', {
        p_cineaste_nom: c.nom,
        p_courant: slots[0],
        p_courant2: slots[1],
        p_courant3: slots[2]
      });
  ...
  // Compte rendu de la cascade renvoyé par la fonction SQL, affiché tel quel :
  // une cascade silencieuse serait impossible à vérifier depuis le back-office.
  var rapport = res.data || {};
  var nb = function(v){ return typeof v === 'number' ? v : 0; };
  var detail = [];
  if(nb(rapport.propositions_maj) || nb(rapport.notifications_maj)){
    detail.push(nb(rapport.propositions_maj) + ' proposition(s) et ' + nb(rapport.notifications_maj) + ' notification(s) reportées');
  }
  if(nb(rapport.propositions_supprimees) || nb(rapport.notifications_supprimees)){
    detail.push(nb(rapport.propositions_supprimees) + ' proposition(s) et ' + nb(rapport.notifications_supprimees) + ' notification(s) supprimées');
  }
  showAdminNotice('Emplacements de « ' + c.nom + ' » enregistrés'
    + (detail.length ? ' — ' + detail.join(', ') + '.' : '.'), true);
  renderCourantFicheResult(c.nom);
  refreshStatsIfActive();
}

// ═══════════════════════════════════════════════════════════════
// STATISTIQUES — cinéastes rattachés à chaque courant / catégorie
// ═══════════════════════════════════════════════════════════════

// Relance le recalcul seulement si l'onglet Statistiques est actuellement
// affiché ; appelé après toute action admin qui change un rattachement
// courant/catégorie (validation, retrait, édition ou suppression du
// catalogue), pour rester à jour sans attendre le prochain tick périodique.
...
async function renderStats(){
  ...
  var cineastes = cineastesRes || [];
  var rows = courantsRows || [];

  // Type de chaque entrée du catalogue : courants et catégories sont classés
  ...
  // du nombre d'affectations, c'est-à-dire de cases courant/courant2/courant3
  // effectivement remplies.
  var countById = {};
  var totals = { fiches: 0, fichesCourant: 0, fichesCategorie: 0, affectations: 0 };
  cineastes.forEach(function(c){
    var vu = { courant: false, categorie: false };
    [c.courant, c.courant2, c.courant3].forEach(function(id){
      if(!id) return;
      countById[id] = (countById[id] || 0) + 1;
      var type = typeById[id];
      if(!type) return; // affectation orpheline : entrée retirée du catalogue
      totals.affectations++;
      vu[type] = true;
    });
    if(vu.courant) totals.fichesCourant++;
    if(vu.categorie) totals.fichesCategorie++;
    if(vu.courant || vu.categorie) totals.fiches++;
  });
  ...
  renderStatsChart(chartEl, items, totals, cineastes.length);
}

function statsFmtNb(n){ return (n || 0).toLocaleString('fr-FR'); }

// Pourcentage à une décimale, espace insécable avant le signe (usage français).
function statsFmtPct(ratio){
  if(!isFinite(ratio)) return '—';
  return (ratio * 100).toLocaleString('fr-FR', { maximumFractionDigits: 1 }) + '\u00a0%';
}

// "1958–1964", "depuis 1958", ou '' si aucune année renseignée au catalogue.
function statsAnnees(it){
  if(!it.debut) return '';
  return it.fin ? it.debut + '–' + it.fin : 'depuis ' + it.debut;
}

// Rendu du graphique unique : bandeau de synthèse (totaux), classement commun
// des courants et des catégories par nombre décroissant de fiches, puis
// échelle et légende. Le type n'est plus une section mais une couleur.
function renderStatsChart(containerEl, items, totals, totalFiches){
  containerEl.innerHTML = '';
  if(!items.length){
    containerEl.innerHTML = '<div class="empty-state">Aucune entrée dans le catalogue.</div>';
    return;
  }
  items.sort(function(a, b){ return b.count - a.count || a.nom.localeCompare(b.nom, 'fr'); });
  var maxCount = Math.max.apply(null, items.map(function(it){ return it.count; })) || 1;
  var utilises = items.filter(function(it){ return it.count > 0; }).length;

  containerEl.appendChild(buildStatsSummary(totals, totalFiches));

  var barsEl = document.createElement('div');
  barsEl.className = 'stats-bars';
  items.forEach(function(it, i){ barsEl.appendChild(buildStatsBar(it, i, maxCount, totalFiches)); });
  containerEl.appendChild(barsEl);

  containerEl.appendChild(buildStatsAxis(maxCount));
  containerEl.appendChild(buildStatsLegend(maxCount, totalFiches, utilises, items.length));
}

// Bandeau supérieur : le total demandé (cinéastes rattachés à au moins une
// entrée du catalogue, courants et catégories confondus) mis en avant, puis sa
// ventilation par famille et le nombre d'affectations.
function buildStatsSummary(totals, totalFiches){
  var wrap = document.createElement('div');
  wrap.className = 'stats-summary';

  var main = document.createElement('div');
  main.className = 'stats-summary-main';
  var mainVal = document.createElement('span');
  mainVal.className = 'stats-summary-value';
  mainVal.textContent = statsFmtNb(totals.fiches);
  var mainLbl = document.createElement('span');
  mainLbl.className = 'stats-summary-label';
  mainLbl.textContent = (totals.fiches > 1 ? 'cinéastes rattachés' : 'cinéaste rattaché')
    + ' à au moins un courant ou une catégorie';
  main.appendChild(mainVal);
  main.appendChild(mainLbl);
  wrap.appendChild(main);

  var kpis = document.createElement('div');
  kpis.className = 'stats-summary-kpis';
  [
    ['', totalFiches ? statsFmtPct(totals.fiches / totalFiches) : '—', 'des ' + statsFmtNb(totalFiches) + ' fiches'],
    ['is-courant', statsFmtNb(totals.fichesCourant), 'dont au moins un courant'],
    ['is-categorie', statsFmtNb(totals.fichesCategorie), 'dont au moins une catégorie'],
    ['', statsFmtNb(totals.affectations), 'affectations au total']
  ].forEach(function(kpiDef){
    var kpi = document.createElement('div');
    kpi.className = 'stats-summary-kpi' + (kpiDef[0] ? ' ' + kpiDef[0] : '');
    var val = document.createElement('span');
    val.className = 'stats-summary-kpi-value';
    val.textContent = kpiDef[1];
    var lbl = document.createElement('span');
    lbl.className = 'stats-summary-kpi-label';
    lbl.textContent = kpiDef[2];
    kpi.appendChild(val);
    kpi.appendChild(lbl);
...
// Une ligne du classement : rang, drapeau du pays d'origine, nom surmontant sa
// famille et sa période, barre proportionnelle colorée selon la famille, puis
// effectif et part du total.
function buildStatsBar(it, index, maxCount, totalFiches){
  var item = document.createElement('div');
  item.className = 'stats-bar-item ' + (it.estCategorie ? 'is-categorie' : 'is-courant');
  if(!it.count) item.className += ' is-zero';
  else if(index < 3) item.className += ' is-top';
  item.title = it.nom + ' — ' + statsFmtNb(it.count) + (it.count > 1 ? ' fiches cinéaste' : ' fiche cinéaste');

  var rank = document.createElement('span');
  rank.className = 'stats-bar-rank';
  rank.textContent = it.count ? String(index + 1) : '—';

  var flagCell = document.createElement('span');
  flagCell.className = 'stats-bar-flagcell';
  flagCell.innerHTML = typeof tcFlagHtml === 'function' ? tcFlagHtml(it.pays, 'stats-bar-flag') : '';
  ...
  if(annees){
    ...
    yearsEl.className = 'stats-bar-years';
    yearsEl.textContent = annees;
  ...
  fill.style.width = (it.count / maxCount * 100).toFixed(2) + '%';
  track.appendChild(fill);

  var count = document.createElement('span');
  count.className = 'stats-bar-count';
  var nb = document.createElement('strong');
  nb.className = 'stats-bar-count-n';
  nb.textContent = statsFmtNb(it.count);
  var pct = document.createElement('span');
  pct.className = 'stats-bar-count-pct';
  pct.textContent = totalFiches ? statsFmtPct(it.count / totalFiches) : '—';
...
// Graduations 0 / ¼ / ½ / ¾ / max, alignées sous la colonne des barres grâce
// à la grille partagée --stats-cols.
function buildStatsAxis(maxCount){
  var axis = document.createElement('div');
  axis.className = 'stats-axis';
  var scale = document.createElement('div');
  scale.className = 'stats-axis-scale';
  [0, 0.25, 0.5, 0.75, 1].forEach(function(ratio){
    var tick = document.createElement('span');
    tick.textContent = statsFmtNb(Math.round(maxCount * ratio));
...
// Pastilles de couleur des deux familles, puis note de lecture des barres.
function buildStatsLegend(maxCount, totalFiches, utilises, catalogue){
  var legend = document.createElement('div');
  legend.className = 'stats-legend';

  var keys = document.createElement('div');
  keys.className = 'stats-legend-keys';
  [['is-courant', 'Courant cinématographique'], ['is-categorie', 'Catégorie']].forEach(function(keyDef){
    var key = document.createElement('span');
    key.className = 'stats-legend-key ' + keyDef[0];
    key.appendChild(document.createElement('i'));
    key.appendChild(document.createTextNode(keyDef[1]));
    keys.appendChild(key);
  });
  legend.appendChild(keys);

  var note = document.createElement('span');
  note.className = 'stats-legend-note';
  note.textContent = 'Longueur des barres proportionnelle au maximum du classement ('
    + statsFmtNb(maxCount) + ' fiches) ; le pourcentage indique la part des '
    + statsFmtNb(totalFiches) + ' fiches cinéaste portant l\'entrée. '
    + statsFmtNb(utilises) + ' des ' + statsFmtNb(catalogue)
    + (catalogue > 1 ? ' entrées du catalogue sont rattachées' : ' entrée du catalogue est rattachée')
    + ' à au moins une fiche.';
...
// ═══════════════════════════════════════════════════════════════
// RÉGLAGES — méthode de calcul du % d'affinité (onglet public "Affinités")
// Stocké dans la table Supabase "app_settings" (clé "compare_affinity_formula"),
// lue par index.js pour choisir la formule à appliquer.
// ═══════════════════════════════════════════════════════════════
var REGLAGES_AFFINITE_DESCRIPTIONS = {
  min: 'Le score est rapporté à la taille du profil le MOINS fourni des deux. Plus généreux quand les deux profils ont des tailles très différentes.',
  max: 'Le score est rapporté à la taille du profil le PLUS fourni des deux. Pénalise structurellement les cinéphiles au profil très complet (ex. un administrateur) dans toutes leurs comparaisons.',
  avg: 'Le score est rapporté à la moyenne des tailles des deux profils. Compromis entre les deux options ci-dessus.',
  jaccard: 'Recouvrement symétrique classique : score commun rapporté au score commun + tout ce qui n\'est PAS partagé chez l\'un ou l\'autre. Ne dépend pas de qui a le plus gros profil.',
  dice: 'Indice de Sørensen-Dice : comme Jaccard mais le score commun compte double au numérateur. Ne dépend pas non plus de qui a le plus gros profil, et donne un pourcentage plus généreux que Jaccard à recouvrement égal.'
};

function updateReglagesAffiniteDesc(){
  var sel = document.getElementById('reglages-affinite-formule');
  var desc = document.getElementById('reglages-affinite-desc');
  if(!sel || !desc) return;
  desc.textContent = REGLAGES_AFFINITE_DESCRIPTIONS[sel.value] || '';
}

async function loadReglagesAffiniteFormule(){
  var sel = document.getElementById('reglages-affinite-formule');
  var errEl = document.getElementById('reglages-error');
  if(!sel) return;
  if(errEl) errEl.style.display = 'none';
  var res;
  try {
  ...
  updateReglagesAffiniteDesc();
}

async function saveReglagesAffiniteFormule(){
  var sel = document.getElementById('reglages-affinite-formule');
  var btn = document.getElementById('reglages-affinite-btn-save');
  if(!sel) return;
  btn.disabled = true;
  var res;
  try {
...
var _reglagesAffiniteSel = document.getElementById('reglages-affinite-formule');
if(_reglagesAffiniteSel) _reglagesAffiniteSel.addEventListener('change', updateReglagesAffiniteDesc);
var _reglagesAffiniteBtnSave = document.getElementById('reglages-affinite-btn-save');
if(_reglagesAffiniteBtnSave) _reglagesAffiniteBtnSave.addEventListener('click', saveReglagesAffiniteFormule);

// ═══════════════════════════════════════════════════════════════
// RÉGLAGES — poids des points communs (score brut, avant passage en %)
// Stockés dans la table Supabase "app_settings" (clés "compare_weight_favfav",
// "compare_weight_otherother", "compare_weight_favother"), lus par index.js.
// ═══════════════════════════════════════════════════════════════
var REGLAGES_AFFINITE_POIDS_DEFAULTS = { compare_weight_favfav: 12, compare_weight_otherother: 5, compare_weight_favother: 3 };
var REGLAGES_AFFINITE_POIDS_FIELDS = [
  { key: 'compare_weight_favfav', id: 'reglages-affinite-w-favfav' },
  { key: 'compare_weight_otherother', id: 'reglages-affinite-w-otherother' },
  { key: 'compare_weight_favother', id: 'reglages-affinite-w-favother' }
];

async function loadReglagesAffinitePoids(){
  var errEl = document.getElementById('reglages-poids-error');
  if(errEl) errEl.style.display = 'none';
  var res;
  try {
    res = await tcWithRetryTimeout(function(){
  ...
  REGLAGES_AFFINITE_POIDS_FIELDS.forEach(function(f){
    var input = document.getElementById(f.id);
    if(!input) return;
    var v = byKey[f.key];
    input.value = (v !== undefined && v !== null && v !== '') ? v : REGLAGES_AFFINITE_POIDS_DEFAULTS[f.key];
  });
}

async function saveReglagesAffinitePoids(){
  var btn = document.getElementById('reglages-affinite-poids-btn-save');
  var errEl = document.getElementById('reglages-poids-error');
  if(errEl) errEl.style.display = 'none';
  var rows = [];
  for(var i=0;i<REGLAGES_AFFINITE_POIDS_FIELDS.length;i++){
    var f = REGLAGES_AFFINITE_POIDS_FIELDS[i];
    var input = document.getElementById(f.id);
    if(!input) continue;
    var num = parseFloat(input.value);
    if(isNaN(num) || num < 0){
      if(errEl){ errEl.textContent = 'Valeur invalide pour "' + f.id + '" — un nombre positif est attendu.'; errEl.style.display = 'block'; }
      return;
    }
    rows.push({ key: f.key, value: String(num), updated_at: new Date().toISOString() });
  }
  if(btn) btn.disabled = true;
  var res;
  try {
...
var _reglagesAffinitePoidsBtnSave = document.getElementById('reglages-affinite-poids-btn-save');
if(_reglagesAffinitePoidsBtnSave) _reglagesAffinitePoidsBtnSave.addEventListener('click', saveReglagesAffinitePoids);
