...
document.querySelectorAll('.tab').forEach(function(tab){
  tab.addEventListener('click', function(){
    ...
    if(tab.getAttribute('data-tab') === 'film-aliases') loadFilmAliasCatalogue();
...
// Un échec ici ne doit jamais bloquer le retrait du courant lui-même.
async function removeCourantActuAndNotif(cineasteNom, courantId){
  ...
  try {
    ...
    if(propRes && !propRes.error && propRes.data){
      ...
      if(propIds.length) await tcWithRetryTimeout(function(){ return sb.from('courant_proposals').delete().in('id', propIds); });
    }
  } catch(e){}
  ...
  try {
    ...
    if(notifRes && !notifRes.error && notifRes.data){
      ...
      if(notifIds.length) await tcWithRetryTimeout(function(){ return sb.from('notifications').delete().in('id', notifIds); });
    }
  } catch(e){}
}

// ═══════════════════════════════════════════════════════════════
// TRADUCTIONS DE FILMS (FR ↔ EN) — utilisé par l'onglet public "Affinités"
// pour reconnaître qu'un même film cité sous deux titres est un point commun.
// ═══════════════════════════════════════════════════════════════
var _filmAliasRows = [];
var _filmAliasSearchTerm = '';

async function loadFilmAliasCatalogue(){
  var listEl = document.getElementById('film-alias-list');
  var errEl = document.getElementById('film-alias-error');
  if(!listEl) return;
  listEl.innerHTML = '<div class="empty-state">Chargement…</div>';
  if(errEl) errEl.style.display = 'none';
  ...
  try {
    res = await tcWithRetryTimeout(function(){
      return sb.from('film_title_aliases').select('id, titre_fr, titre_en').order('titre_fr', { ascending: true });
    });
  } catch(err){
    if(errEl){ errEl.textContent = 'Erreur de chargement : ' + (err && err.message ? err.message : err); errEl.style.display = 'block'; }
    listEl.innerHTML = '';
  ...
  _filmAliasRows = res.data || [];
  renderFilmAliasCatalogue();
}

function renderFilmAliasCatalogue(){
  var listEl = document.getElementById('film-alias-list');
  if(!listEl) return;
  listEl.innerHTML = '';
  var term = normStr(_filmAliasSearchTerm.trim());
  var rows = !term ? _filmAliasRows : _filmAliasRows.filter(function(r){
    return normStr(r.titre_fr).indexOf(term) !== -1 || normStr(r.titre_en).indexOf(term) !== -1;
  });
  if(!rows.length){
    listEl.innerHTML = '<div class="empty-state">' + (_filmAliasRows.length ? 'Aucun résultat pour cette recherche.' : 'Aucune traduction enregistrée.') + '</div>';
    return;
  }
  rows.forEach(function(row){
    var rowEl = document.createElement('div');
    rowEl.className = 'film-alias-row';

    var frInput = document.createElement('input');
    frInput.type = 'text'; frInput.className = 'autocomplete-input'; frInput.value = row.titre_fr;

    var enInput = document.createElement('input');
    enInput.type = 'text'; enInput.className = 'autocomplete-input'; enInput.value = row.titre_en;

    var actionsCell = document.createElement('div');
    actionsCell.className = 'film-alias-row-actions';
    var saveBtn = document.createElement('button');
    saveBtn.className = 'btn-primary';
    saveBtn.textContent = 'Enregistrer';
    saveBtn.addEventListener('click', function(){
      saveFilmAliasEdits(row.id, frInput.value.trim(), enInput.value.trim(), saveBtn);
    });
    var delBtn = document.createElement('button');
    delBtn.className = 'btn-reject';
    delBtn.textContent = 'Supprimer';
    delBtn.addEventListener('click', function(){
      showConfirmModal('Supprimer la traduction « ' + row.titre_fr + ' / ' + row.titre_en + ' » ?', function(){
        deleteFilmAlias(row.id, delBtn);
      });
    });
    actionsCell.appendChild(saveBtn);
    actionsCell.appendChild(delBtn);
...
async function saveFilmAliasEdits(id, titreFr, titreEn, btn){
  if(!titreFr || !titreEn){ showAdminNotice('Les deux titres sont obligatoires.', false); return; }
  if(btn) btn.disabled = true;
  var res;
  try {
    res = await tcWithRetryTimeout(function(){
      return sb.from('film_title_aliases').update({ titre_fr: titreFr, titre_en: titreEn }).eq('id', id).select('id');
    });
  } catch(err){ showAdminNotice('Erreur : ' + (err && err.message ? err.message : err), false); if(btn) btn.disabled = false; return; }
  if(res.error){
    var msg1 = /duplicate|unique/i.test(res.error.message || '') ? 'Ce titre français existe déjà.' : res.error.message;
    showAdminNotice('Erreur : ' + msg1, false);
    if(btn) btn.disabled = false;
    return;
  }
  if(btn) btn.disabled = false;
  showAdminNotice('Traduction mise à jour.', true);
  loadFilmAliasCatalogue();
}

async function deleteFilmAlias(id, btn){
  if(btn) btn.disabled = true;
  var res;
  try {
    res = await tcWithRetryTimeout(function(){ return sb.from('film_title_aliases').delete().eq('id', id).select('id'); }, { retries: 0 });
  } catch(err){ showAdminNotice('Erreur : ' + (err && err.message ? err.message : err), false); if(btn) btn.disabled = false; return; }
  if(res.error){ showAdminNotice('Erreur : ' + res.error.message, false); if(btn) btn.disabled = false; return; }
  if(!res.data || !res.data.length){ showAdminNotice('La suppression n\'a pas abouti — vérifiez les droits de la table film_title_aliases.', false); if(btn) btn.disabled = false; return; }
  showAdminNotice('Traduction supprimée.', true);
  loadFilmAliasCatalogue();
}

async function addFilmAlias(){
  var frInput = document.getElementById('film-alias-fr');
  var enInput = document.getElementById('film-alias-en');
  var btn = document.getElementById('film-alias-btn-add');
  var errEl = document.getElementById('film-alias-error');
  var titreFr = frInput.value.trim();
  var titreEn = enInput.value.trim();
  if(errEl) errEl.style.display = 'none';

  if(!titreFr){ showAdminNotice('Le titre français est obligatoire.', false); return; }
  if(!titreEn){ showAdminNotice('Le titre anglais est obligatoire.', false); return; }

  btn.disabled = true;
  var res;
  try {
    res = await tcWithRetryTimeout(function(){
      return sb.from('film_title_aliases').insert({ titre_fr: titreFr, titre_en: titreEn }).select('id');
    }, { retries: 0 });
  } catch(err){
    showAdminNotice('Erreur : ' + (err && err.message ? err.message : err), false);
    btn.disabled = false;
  ...
  btn.disabled = false;
  if(res.error){
    var msg2 = /duplicate|unique/i.test(res.error.message || '') ? 'Ce titre français existe déjà dans le dictionnaire.' : res.error.message;
    showAdminNotice('Erreur : ' + msg2, false);
    return;
  }
  if(!res.data || !res.data.length){ showAdminNotice('L\'ajout n\'a pas abouti — vérifiez les droits de la table film_title_aliases.', false); return; }

  frInput.value = '';
  enInput.value = '';
  showAdminNotice('Traduction « ' + titreFr + ' / ' + titreEn + ' » ajoutée.', true);
  loadFilmAliasCatalogue();
}

var _filmAliasBtnAdd = document.getElementById('film-alias-btn-add');
if(_filmAliasBtnAdd) _filmAliasBtnAdd.addEventListener('click', addFilmAlias);

var _filmAliasSearchInput = document.getElementById('film-alias-search');
if(_filmAliasSearchInput){
  var _filmAliasSearchDebounce = null;
  _filmAliasSearchInput.addEventListener('input', function(e){
    _filmAliasSearchTerm = e.target.value;
    clearTimeout(_filmAliasSearchDebounce);
    _filmAliasSearchDebounce = setTimeout(renderFilmAliasCatalogue, 200);
  });
}
...
async function loadReglagesAffiniteFormule(){
  ...
  sel.value = (res.data && res.data.value) || 'min';
