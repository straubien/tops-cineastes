...
function loadAllCineastesAdmin(offset, pageSize){
  return sb.from('cineastes').select('nom,fbid,url_facebook,duo,naissance,deces,vivant,tops_contributeurs,pays')
    .order('id', { ascending: true })
    .range(offset, offset + pageSize - 1)
    .then(function(res){
      var rows = res.data || [];
      if(rows.length === pageSize){
        return loadAllCineastesAdmin(offset + pageSize, pageSize).then(function(more){ return rows.concat(more); });
      }
      return rows;
    });
}
...
async function loadSubmissions(){
  ...
  if(truncWarn){
    if(totalCount !== null && totalCount > SUBMISSIONS_LIMIT){
      truncWarn.textContent = 'Affichage des ' + SUBMISSIONS_LIMIT + ' soumissions les plus récentes sur ' + totalCount + ' au total. Validez ou refusez les plus anciennes pour les faire apparaître.';
...
async function loadCommentsAdmin(){
  ...
  try {
    ...
    var nameById = {};
    if(contribRes.error){
      errEl.textContent = 'Erreur de chargement des contributeurs : ' + friendlyError(contribRes.error);
    ...
    errEl.textContent = 'Erreur de chargement : ' + friendlyError(err);
    errEl.style.display = 'block';
    container.innerHTML = '';
  }
}
...
function renderFlagsAdmin(){
  ...
  if(rows.length === 200){
    ...
    infoTd.textContent = 'Affichage limité aux 200 premiers résultats — affinez la recherche pour voir les autres.';
