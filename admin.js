...
// ── Config TMDB (matching des titres de films) ──────────────────
var TMDB_API_KEY = 'fea3cb32bf2802cf15b71898983593ae';
...
document.querySelectorAll('.tab').forEach(function(tab){
  tab.addEventListener('click', function(){
    ...
    if(tab.getAttribute('data-tab') === 'tmdb') loadTmdbMatches();
    if(tab.getAttribute('data-tab') === 'ranking-films') loadRankingFilms();
  });
});
...
async function renderDashboard(force){
  ...
  document.getElementById('dash-total-cineastes').textContent = _totalCineastes.toLocaleString('fr-FR') + t('adm_cineastes_indexes');
...
// Ajoute le courant proposé à la liste existante du cinéaste (format "A | B"),
// sans doublon, puis marque la proposition comme validée.
async function approveCourant(row, apprBtn, rejBtn){
  ...
  try {
    cineasteRes = await tcWithRetryTimeout(function(){
      return sb.from('cineastes').select('nom, courant').eq('nom', row.cineaste_nom).single();
    });
  } catch(err){ showAdminNotice('Erreur : ' + (err && err.message ? err.message : err), false); reenable(); return; }
  if(cineasteRes.error || !cineasteRes.data){ showAdminNotice('Cinéaste introuvable — validation annulée.', false); reenable(); return; }

  var existants = (cineasteRes.data.courant || '').split(' | ').map(function(s){ return s.trim(); }).filter(Boolean);
  var dejaPresent = existants.some(function(c){ return c.toLowerCase() === row.courant.toLowerCase(); });
  var nouveauCourant = dejaPresent ? cineasteRes.data.courant : existants.concat([row.courant]).join(' | ');

  var updRes;
  try {
    updRes = await tcWithRetryTimeout(function(){
      return sb.from('cineastes').update({ courant: nouveauCourant }).eq('nom', row.cineaste_nom).select('nom');
    });
  } catch(err){ showAdminNotice('Erreur : ' + (err && err.message ? err.message : err), false); reenable(); return; }
...
// ═══════════════════════════════════════════════════════════════
// MATCHING TMDB — association des titres saisis (tous formats/langues)
// aux fiches films TMDB, via la filmographie du cinéaste concerné.
// ═══════════════════════════════════════════════════════════════

var _tmdbFilter = 'a_valider'; // filtre actif de la liste de revue
var _tmdbAvalSub = 'all';     // sous-filtre pour a_valider : all / with_match / no_match
var _tmdbRunning = false;
var _tmdbCancelRequested = false;
// Cache mémoire (durée de vie = un run de matching) : movieId -> [titres connus]
var _tmdbTranslationsCache = {};

document.querySelectorAll('[data-tmdb-filter]').forEach(function(btn){
  btn.addEventListener('click', function(){
    _tmdbFilter = btn.getAttribute('data-tmdb-filter');
    document.querySelectorAll('[data-tmdb-filter]').forEach(function(b){ b.classList.remove('active'); });
    btn.classList.add('active');
    var avalControls = document.getElementById('tmdb-aval-controls');
    if(avalControls) avalControls.style.display = (_tmdbFilter === 'a_valider' || _tmdbFilter === 'en_attente') ? 'flex' : 'none';
    loadTmdbMatches();
  });
});

document.querySelectorAll('[data-aval-sub]').forEach(function(btn){
  btn.addEventListener('click', function(){
    _tmdbAvalSub = btn.getAttribute('data-aval-sub');
    document.querySelectorAll('[data-aval-sub]').forEach(function(b){ b.classList.remove('active'); });
    btn.classList.add('active');
    loadTmdbMatches();
  });
});

var _btnBulkAccept = document.getElementById('btn-tmdb-bulk-accept');
if(_btnBulkAccept) _btnBulkAccept.addEventListener('click', function(){
  var scoreInput = document.getElementById('tmdb-bulk-score');
  var statusEl = document.getElementById('tmdb-bulk-status');
  var threshold = parseInt(scoreInput ? scoreInput.value : 70, 10);
  if(isNaN(threshold) || threshold < 1 || threshold > 99){ if(statusEl) statusEl.textContent = 'Seuil invalide.'; return; }
  var msg = 'Accepter en lot tous les films "à valider" avec une suggestion TMDB et un score ≥ ' + threshold + '% ? Cette action est irréversible sans intervention manuelle.';
  showConfirmModal(msg, async function(){
    if(statusEl) statusEl.textContent = 'Mise à jour en cours…';
    if(_btnBulkAccept) _btnBulkAccept.disabled = true;
    try {
      var res = await tcWithRetryTimeout(function(){
        return sb.from('film_tmdb_matches')
          .update({ statut: 'manuel' })
          .eq('statut', 'a_valider')
          .gte('score', threshold / 100)
          .not('tmdb_id', 'is', null)
          .select('id', { count: 'exact', head: true });
      });
      if(res && res.error) throw new Error(res.error.message);
      var n = res && res.count != null ? res.count : '?';
      if(statusEl) statusEl.textContent = n + ' film(s) acceptés.';
      showAdminNotice(n + ' film(s) acceptés en lot (score ≥ ' + threshold + '%).', true);
      loadTmdbMatches();
    } catch(err){
      if(statusEl) statusEl.textContent = 'Erreur : ' + (err && err.message ? err.message : err);
    } finally {
      if(_btnBulkAccept) _btnBulkAccept.disabled = false;
    }
  });
});

var _btnTmdbRun = document.getElementById('btn-tmdb-run');
if(_btnTmdbRun) _btnTmdbRun.addEventListener('click', function(){
  if(_tmdbRunning) return;
  runTmdbMatchingBatch();
});

var _btnTmdbStop = document.getElementById('btn-tmdb-stop');
if(_btnTmdbStop) _btnTmdbStop.addEventListener('click', function(){
  if(!_tmdbRunning) return;
  _tmdbCancelRequested = true;
  _btnTmdbStop.disabled = true;
  var progressEl = document.getElementById('tmdb-run-progress');
  if(progressEl) progressEl.textContent += ' — arrêt demandé, fin du film en cours puis écriture des résultats déjà calculés…';
});

// ── Appels TMDB ───────────────────────────────────────────────
function tmdbSleep(ms){ return new Promise(function(resolve){ setTimeout(resolve, ms); }); }

async function tmdbFetchJson(path, params){
  var url = TMDB_API_BASE + path + (path.indexOf('?') === -1 ? '?' : '&')
    + 'api_key=' + encodeURIComponent(TMDB_API_KEY) + '&language=fr-FR';
  if(params){
    Object.keys(params).forEach(function(k){
      if(params[k] !== undefined && params[k] !== null && params[k] !== '') url += '&' + k + '=' + encodeURIComponent(params[k]);
    });
  }
  var res = await tcFetchWithTimeout(url, { timeoutMs: 12000, retries: 1 });
  if(!res.ok){
    var err = new Error('TMDB HTTP ' + res.status);
    err.tmdbStatus = res.status;
    throw err;
  }
  return res.json();
}

// "NOM, Prénom & NOM2, Prénom2" (format des cinéastes du site) → nom de recherche
// TMDB pour la première personne du duo (une œuvre commune apparaît dans la
// filmographie de chacun des deux réalisateurs).
function tmdbPersonSearchName(cineasteNom){
  var first = (cineasteNom || '').split('&')[0].trim().replace(/,\s*$/, '');
...
// Normalisation d'un titre pour comparaison floue : accents/casse retirés
// (normStr, utils.js), ponctuation neutralisée, article initial supprimé.
function tmdbNormTitle(s){
  if(!s) return '';
  var n = normStr(String(s)).replace(/[^a-z0-9]+/g, ' ').trim();
  // Articles initiaux : FR, EN, DE, ES, IT, PT + formes courtes
  n = n.replace(/^(the|a|an|le|la|les|l|un|une|des|du|de|der|die|das|ein|eine|el|los|las|lo|un|una|il|gli|i|os|as|um|uma)\s+/, '');
  return n.replace(/\s+/g, ' ').trim();
}

function tmdbBigrams(s){
  var out = [];
  for(var i = 0; i < s.length - 1; i++) out.push(s.substr(i, 2));
  return out;
}

// Coefficient de Dice sur bigrammes — mesure de similarité de chaînes sans
// dépendance externe, tolérante aux variations mineures d'orthographe.
function tmdbDiceScore(a, b){
  var na = tmdbNormTitle(a), nb = tmdbNormTitle(b);
  if(!na || !nb) return 0;
  if(na === nb) return 1;
  var A = tmdbBigrams(na), B = tmdbBigrams(nb);
  if(!A.length || !B.length) return na === nb ? 1 : 0;
  var matches = 0;
  var pool = B.slice();
  A.forEach(function(bg){
    var idx = pool.indexOf(bg);
    if(idx !== -1){ matches++; pool.splice(idx, 1); }
  });
  return (2 * matches) / (A.length + B.length);
}

// Résout (et mémorise en base) l'identifiant TMDB "person" d'un cinéaste.
async function resolveCineasteTmdbPersonId(cineasteNom, naissanceAnnee){
  var existing = await tcWithRetryTimeout(function(){
    return sb.from('cineastes').select('tmdb_person_id').eq('nom', cineasteNom).single();
  });
  if(existing && existing.data && existing.data.tmdb_person_id) return existing.data.tmdb_person_id;

  var searchName = tmdbPersonSearchName(cineasteNom);
  if(!searchName) return null;
  var data;
  try {
    data = await tmdbFetchJson('/search/person', { query: searchName });
  } catch(e){ return null; }
  var results = (data && data.results) || [];
  if(!results.length) return null;

  // Priorité aux réalisateurs (évite de confondre avec un acteur homonyme célèbre).
  var directors = results.filter(function(r){ return r.known_for_department === 'Directing'; });
  var best = directors.length ? directors[0] : results[0];

  await tcWithRetryTimeout(function(){
    return sb.from('cineastes').update({ tmdb_person_id: best.id }).eq('nom', cineasteNom).select('nom');
  });
  return best.id;
}

// Filmographie "réalisateur" d'une personne TMDB (dédupliquée par film).
async function tmdbGetDirectorFilmography(personId){
  var data;
  try {
    data = await tmdbFetchJson('/person/' + personId + '/movie_credits');
  } catch(e){ return []; }
  var crew = (data && data.crew) || [];
  var seen = {};
  var out = [];
  crew.forEach(function(c){
    if(c.job !== 'Director' && c.job !== 'Co-Director') return;
    if(seen[c.id]) return;
    seen[c.id] = true;
    out.push({
      id: c.id,
      ...
      annee: c.release_date ? c.release_date.slice(0, 4) : ''
    });
  });
  return out;
}

// Récupère (avec cache mémoire) les titres utiles d'un film : titre FR (fourni
// par l'API en language=fr-FR), titre original, et traductions EN + langue originale.
// On évite de charger les 40+ autres langues qui n'apportent que du bruit.
async function tmdbGetAllTitles(movie){
  if(_tmdbTranslationsCache[movie.id]) return _tmdbTranslationsCache[movie.id];
  var titles = [movie.title, movie.original_title].filter(Boolean);
  try {
    var data = await tmdbFetchJson('/movie/' + movie.id + '/translations');
    var translations = (data && data.translations) || [];
    var keepLangs = { fr: true, en: true };
    if(movie.original_language) keepLangs[movie.original_language] = true;
    translations.forEach(function(tr){
      if(tr.data && tr.data.title && keepLangs[tr.iso_639_1]) titles.push(tr.data.title);
    });
  } catch(e){ /* best-effort */ }
  await tmdbSleep(100);
  _tmdbTranslationsCache[movie.id] = titles;
  return titles;
}

// Compare un titre saisi à la filmographie candidate et retourne la meilleure
// correspondance { movie, score } (score dans [0,1], ajusté par l'année).
// Optimisation : on ne fetche les traductions que si le pré-score (title +
// original_title uniquement) dépasse 0.25 — évite des appels API inutiles
// pour les films clairement hors sujet.
async function tmdbMatchFilm(titreSaisi, anneeSaisie, filmography){
  var best = null, bestScore = -1;
  for(var i = 0; i < filmography.length; i++){
    var movie = filmography[i];
    // Pré-score rapide sans appel réseau
    var preScore = Math.max(
      tmdbDiceScore(titreSaisi, movie.title),
      tmdbDiceScore(titreSaisi, movie.original_title)
    );
    var titles;
    if(preScore >= 0.25){
      titles = await tmdbGetAllTitles(movie); // fetch FR/EN/original translations
    } else {
      titles = [movie.title, movie.original_title].filter(Boolean);
    }
    var score = 0;
    titles.forEach(function(ti){ score = Math.max(score, tmdbDiceScore(titreSaisi, ti)); });
    if(anneeSaisie && movie.annee){
      var diff = Math.abs(parseInt(anneeSaisie, 10) - parseInt(movie.annee, 10));
      if(!isNaN(diff)){
        if(diff <= 1)      score = Math.min(1, score + 0.05);  // bonus : même année
        else if(diff <= 2) { /* écart de 2 ans : neutre (sorties régionales) */ }
        else               score = Math.max(0, score - 0.20);  // malus léger au-delà
      }
    }
    if(score > bestScore){ bestScore = score; best = movie; }
  }
  if(!best) return { movie: null, score: 0 };
  return { movie: best, score: bestScore };
}

// ── Traitement en lot ─────────────────────────────────────────
async function runTmdbMatchingBatch(){
  _tmdbRunning = true;
  _tmdbCancelRequested = false;
  _tmdbTranslationsCache = {};
  var btn = document.getElementById('btn-tmdb-run');
  var stopBtn = document.getElementById('btn-tmdb-stop');
  var progressEl = document.getElementById('tmdb-run-progress');
  var errEl = document.getElementById('tmdb-run-error');
  if(btn) btn.disabled = true;
  if(stopBtn){ stopBtn.disabled = false; stopBtn.style.display = 'inline-block'; }
  if(errEl) errEl.style.display = 'none';
  function progress(msg){ if(progressEl) progressEl.textContent = msg; }

  try {
    progress('Chargement des tops publiés (table tops + soumissions approuvées)…');

    // normApo : apostrophe typographique ' (U+2019) → droite ' (U+0027).
    // Défini ici pour être disponible dans tous les chargeurs ci-dessous,
    // et éviter que le même cinéaste soit compté deux fois sous deux variantes.
    function normApo(s){ return s ? s.replace(/’/g, "'") : s; }

    // 1) Films uniques par cinéaste, à partir des tops publiés.
    var byCineaste = {};
    async function loadAllTops(offset, pageSize){
      var res = await tcWithRetryTimeout(function(){
        return sb.from('tops').select('cineaste_nom, films').order('id', { ascending: true }).range(offset, offset + pageSize - 1);
      });
      if(res.error) throw new Error(res.error.message);
      var rows = res.data || [];
      rows.forEach(function(row){
        if(!row.cineaste_nom || !Array.isArray(row.films)) return;
        var cn = normApo(row.cineaste_nom);
        if(!byCineaste[cn]) byCineaste[cn] = {};
        row.films.forEach(function(f){
          if(!f || !f.titre) return;
          var key = f.titre.trim() + '|||' + (f.annee || '').toString().trim();
          byCineaste[cn][key] = { titre: f.titre.trim(), annee: (f.annee || '').toString().trim() };
        });
      });
      if(rows.length === pageSize) await loadAllTops(offset + pageSize, pageSize);
    }
    await loadAllTops(0, 1000);

    // 1bis) Idem pour les soumissions approuvées : le site affiche ces tops
    // directement depuis `submissions` (status=approved), sans forcément les
    // recopier dans `tops` — il faut les inclure pour ne pas rater de films.
    async function loadAllApprovedSubmissions(offset, pageSize){
      var res = await tcWithRetryTimeout(function(){
        return sb.from('submissions').select('parsed_json').eq('status', 'approved')
          .order('submitted_at', { ascending: true }).range(offset, offset + pageSize - 1);
      });
      if(res.error) throw new Error(res.error.message);
      var rows = res.data || [];
      rows.forEach(function(row){
        var pj = row.parsed_json;
        if(!pj || !pj.cineaste || !Array.isArray(pj.films)) return;
        var cn = normApo(pj.cineaste);
        if(!byCineaste[cn]) byCineaste[cn] = {};
        pj.films.forEach(function(f){
          if(!f || !f.titre) return;
          var key = f.titre.trim() + '|||' + (f.annee || '').toString().trim();
          byCineaste[cn][key] = { titre: f.titre.trim(), annee: (f.annee || '').toString().trim() };
        });
      });
      if(rows.length === pageSize) await loadAllApprovedSubmissions(offset + pageSize, pageSize);
    }
    await loadAllApprovedSubmissions(0, 1000);

    // 1ter) Tops statiques Muzard et Cnudde (non stockés dans Supabase).
    // Ces deux contributeurs fondateurs n'ont pas de soumissions dans la base ;
    // leurs tops vivent uniquement dans muzard.json et cnudde.json.
    async function loadJsonTops(url){
      try {
        var data = await tcFetchWithTimeout(url).then(function(r){ return r.json(); });
        if(!Array.isArray(data.tops)) return;
        data.tops.forEach(function(top){
          var cn = normApo(top.cineaste);
          if(!cn || !Array.isArray(top.films)) return;
          if(!byCineaste[cn]) byCineaste[cn] = {};
          top.films.forEach(function(f){
            if(!f || !f.titre) return;
            var key = f.titre.trim() + '|||' + (f.annee || '').toString().trim();
            byCineaste[cn][key] = { titre: f.titre.trim(), annee: (f.annee || '').toString().trim() };
          });
        ...
        console.warn('TMDB batch: impossible de charger', url, e);
      }
    }
    await loadJsonTops('muzard.json');
    await loadJsonTops('cnudde.json');

    // 2) Correspondances déjà tranchées par un admin : ne pas les ré-écraser.
    var decided = {};
    async function loadDecided(offset, pageSize){
      var res = await tcWithRetryTimeout(function(){
        return sb.from('film_tmdb_matches').select('cineaste_nom, titre_saisi, annee_saisie, statut')
          .range(offset, offset + pageSize - 1);
      });
      if(res.error) throw new Error(res.error.message);
      var rows = res.data || [];
      rows.forEach(function(r){
        decided[normApo(r.cineaste_nom) + '|||' + r.titre_saisi + '|||' + (r.annee_saisie || '')] = true;
      });
      if(rows.length === pageSize) await loadDecided(offset + pageSize, pageSize);
    }
    await loadDecided(0, 1000);

    // 3) Cinéastes -> tmdb_person_id + naissance (pour désambiguïsation).
    // Paginé : la table dépasse 3000 lignes, PostgREST plafonne à ~1000/requête.
    var cineasteMeta = {};
    async function loadCineasteMeta(offset, pageSize){
      var res = await tcWithRetryTimeout(function(){
        return sb.from('cineastes').select('nom, naissance, tmdb_person_id')
          .order('id', { ascending: true }).range(offset, offset + pageSize - 1);
      });
      if(res.error) throw new Error(res.error.message);
      var rows = res.data || [];
      rows.forEach(function(c){ cineasteMeta[normApo(c.nom)] = c; });
      if(rows.length === pageSize) await loadCineasteMeta(offset + pageSize, pageSize);
    }
    await loadCineasteMeta(0, 1000);

    // Dédoublonnage des duos : les JSON peuvent utiliser une forme longue
    // ("CONNOLLY, Bob & ANDERSON, Robin") alors que cineastes.nom stocke la
    // forme courte ("CONNOLLY & ANDERSON") ou une variante accentuée
    // ("DELÉPINE" vs "DELEPINE"). On cherche la forme canonique en comparant
    // les noms de famille normalisés (sans accents, sans casse).
    var metaKeysForDuos = Object.keys(cineasteMeta);
    var duoMerges = {};
    Object.keys(byCineaste).forEach(function(cn){
      if(cineasteMeta[cn] || cn.indexOf(' & ') === -1) return;
      var parts = cn.split(' & ');
      if(parts.length !== 2) return;
      var s1 = normStr(parts[0].split(',')[0].trim());
      var s2 = normStr(parts[1].split(',')[0].trim());
      for(var i = 0; i < metaKeysForDuos.length; i++){
        var mk = metaKeysForDuos[i];
        if(mk.indexOf(' & ') !== -1){
          var mkn = normStr(mk);
          if(mkn.indexOf(s1) !== -1 && mkn.indexOf(s2) !== -1){ duoMerges[cn] = mk; break; }
        }
      }
    });
    Object.keys(duoMerges).forEach(function(alias){
      var canonical = duoMerges[alias];
      if(!byCineaste[canonical]) byCineaste[canonical] = {};
      var films = byCineaste[alias];
      Object.keys(films).forEach(function(k){ byCineaste[canonical][k] = films[k]; });
      delete byCineaste[alias];
    });

    var cineasteNoms = Object.keys(byCineaste);
    var totalCineastes = cineasteNoms.length;
    var upserts = [];
    var nDone = 0, nMatched = 0, nSkippedNoPerson = 0;
    var wasCancelled = false;

    for(var ci = 0; ci < cineasteNoms.length; ci++){
      if(_tmdbCancelRequested){ wasCancelled = true; break; }
      var cineasteNom = cineasteNoms[ci];
      var filmsToMatch = Object.keys(byCineaste[cineasteNom])
        .map(function(k){ return byCineaste[cineasteNom][k]; })
        .filter(function(f){ return !decided[cineasteNom + '|||' + f.titre + '|||' + f.annee]; });

      progress('Cinéaste ' + (ci + 1) + '/' + totalCineastes + ' — ' + cineasteNom + ' (' + filmsToMatch.length + ' films à traiter)');

      if(!filmsToMatch.length){ nDone++; continue; }

      var meta = cineasteMeta[normApo(cineasteNom)];
      var naissanceAnnee = meta && Array.isArray(meta.naissance) ? meta.naissance[0] : (meta ? meta.naissance : null);
      var personId = meta && meta.tmdb_person_id;
      if(!personId){
        personId = await resolveCineasteTmdbPersonId(cineasteNom, naissanceAnnee);
        await tmdbSleep(150);
      }

      if(!personId){
        nSkippedNoPerson++;
        filmsToMatch.forEach(function(f){
          upserts.push({
            cineaste_nom: cineasteNom, titre_saisi: f.titre, annee_saisie: f.annee,
            tmdb_id: null, tmdb_titre: null, tmdb_annee: null, score: 0,
            statut: 'a_valider', matched_at: new Date().toISOString()
          });
        });
        nDone++;
        continue;
      ...
      var filmography = await tmdbGetDirectorFilmography(personId);
      await tmdbSleep(150);

      for(var fi = 0; fi < filmsToMatch.length; fi++){
        if(_tmdbCancelRequested){ wasCancelled = true; break; }
        var f = filmsToMatch[fi];
        var match = await tmdbMatchFilm(f.titre, f.annee, filmography);
        var statut = match.movie && match.score >= 0.80 ? 'auto' : 'a_valider';
        if(match.movie && match.score >= 0.80) nMatched++;
        upserts.push({
          cineaste_nom: cineasteNom, titre_saisi: f.titre, annee_saisie: f.annee,
          tmdb_id: match.movie ? match.movie.id : null,
          tmdb_titre: match.movie ? (match.movie.title || match.movie.original_title) : null,
          tmdb_annee: match.movie ? match.movie.annee : null,
          score: Math.round(match.score * 100) / 100,
          statut: statut,
          matched_at: new Date().toISOString()
        });
      }
      if(!wasCancelled) nDone++;

      // Écriture par lots de 200 pour ne pas garder des milliers de lignes en mémoire.
      if(upserts.length >= 200){
        await tcWithRetryTimeout(function(){
          return sb.from('film_tmdb_matches').upsert(upserts, { onConflict: 'cineaste_nom,titre_saisi,annee_saisie' });
        });
        upserts = [];
      }
    ...
    if(wasCancelled){
      progress('Arrêté par l\'utilisateur — ' + nDone + ' cinéaste(s) traités, ' + nMatched + ' film(s) associés automatiquement. Les résultats déjà calculés ont été enregistrés ; relancez pour continuer là où ça s\'est arrêté.');
      showAdminNotice('Matching TMDB arrêté — résultats partiels enregistrés.', true);
    } else {
      progress('Terminé — ' + nDone + ' cinéaste(s) traités, ' + nMatched + ' film(s) associés automatiquement'
        + (nSkippedNoPerson ? ', ' + nSkippedNoPerson + ' cinéaste(s) introuvables sur TMDB.' : '.'));
      showAdminNotice('Matching TMDB terminé.', true);
    }
    loadTmdbMatches();
  } catch(err){
    if(errEl){ errEl.textContent = 'Erreur : ' + (err && err.message ? err.message : err); errEl.style.display = 'block'; }
...
// ── Liste de revue (validation manuelle) ────────────────────────
var _tmdbFilterLabels = { a_valider: 'À valider', en_attente: 'En attente', auto: 'Associés auto', manuel: 'Validés manuellement', rejete: 'Rejetés' };

async function updateTmdbFilterCounts(){
  var statuts = ['a_valider', 'en_attente', 'auto', 'manuel', 'rejete'];
  var counts = await Promise.all(statuts.map(function(s){
    return tcWithRetryTimeout(function(){
      return sb.from('film_tmdb_matches').select('id', { count: 'exact', head: true }).eq('statut', s);
    }).then(function(r){ return r && r.count != null ? r.count : '?'; }).catch(function(){ return '?'; });
  }));
  statuts.forEach(function(s, i){
    var btn = document.querySelector('[data-tmdb-filter="' + s + '"]');
    if(btn) btn.textContent = _tmdbFilterLabels[s] + ' (' + counts[i] + ')';
  });
}

async function loadTmdbMatches(){
  var listEl = document.getElementById('tmdb-list');
  var errEl = document.getElementById('tmdb-error');
  if(!listEl) return;
  listEl.innerHTML = '<div class="empty-state">Chargement…</div>';
  if(errEl) errEl.style.display = 'none';
  var avalControls = document.getElementById('tmdb-aval-controls');
  var isGroupedFilter = _tmdbFilter === 'a_valider' || _tmdbFilter === 'en_attente';
  if(avalControls) avalControls.style.display = isGroupedFilter ? 'flex' : 'none';

  updateTmdbFilterCounts();

  var res;
  try {
    res = await tcWithRetryTimeout(function(){
      var q = sb.from('film_tmdb_matches').select('*').eq('statut', _tmdbFilter);
      if(_tmdbFilter === 'a_valider' || _tmdbFilter === 'en_attente'){
        if(_tmdbAvalSub === 'with_match') q = q.not('tmdb_id', 'is', null);
        else if(_tmdbAvalSub === 'no_match') q = q.is('tmdb_id', null);
        // Groupé par cinéaste puis score décroissant dans chaque groupe
        q = q.order('cineaste_nom', { ascending: true }).order('score', { ascending: false });
        return q.limit(3000);
      } else {
        q = q.order('score', { ascending: true });
        return q.limit(300);
      }
    });
  } catch(err){
  ...
  renderTmdbMatches(res.data || []);
}

function renderTmdbMatches(rows){
  var listEl = document.getElementById('tmdb-list');
  listEl.innerHTML = '';
  if(!rows.length){
    listEl.innerHTML = '<div class="empty-state">Aucun film dans cette catégorie.</div>';
    return;
  }
  if(_tmdbFilter === 'a_valider' || _tmdbFilter === 'en_attente'){
    renderTmdbMatchesGrouped(rows);
  } else {
    rows.forEach(function(row){ listEl.appendChild(buildTmdbMatchCard(row)); });
  }
}

function renderTmdbMatchesGrouped(rows){
  var listEl = document.getElementById('tmdb-list');
  var groups = {}, order = [];
  rows.forEach(function(row){
    var cn = row.cineaste_nom;
    if(!groups[cn]){ groups[cn] = []; order.push(cn); }
    groups[cn].push(row);
  });
  var total = order.length;
  var info = document.createElement('div');
  info.style.cssText = 'font-size:12px;color:var(--text-muted,#888);margin-bottom:10px;';
  info.textContent = total + ' cinéaste' + (total > 1 ? 's' : '') + ' — cliquer sur un nom pour afficher ses films';
  listEl.appendChild(info);
  order.forEach(function(cn){ listEl.appendChild(buildDirectorSection(cn, groups[cn])); });
}

function buildDirectorSection(directorName, films){
  var withMatch = films.filter(function(f){ return f.tmdb_id; }).length;
  var section = document.createElement('div');
  section.style.cssText = 'margin-bottom:8px;border:1px solid var(--border,#333);border-radius:8px;overflow:hidden;';

  var header = document.createElement('div');
  header.style.cssText = 'padding:10px 14px;background:var(--bg-card,#1a1a1a);display:flex;justify-content:space-between;align-items:center;cursor:pointer;gap:10px;flex-wrap:wrap;user-select:none;';

  var info = document.createElement('div');
  info.style.cssText = 'font-weight:600;font-size:13px;flex:1;min-width:0;';
  info.textContent = directorName + ' — ' + films.length + ' film' + (films.length > 1 ? 's' : '')
    + (withMatch < films.length ? ' (' + withMatch + ' avec suggestion)' : '');

  var actions = document.createElement('div');
  actions.style.cssText = 'display:flex;gap:6px;flex-shrink:0;';

  var btnAccept = document.createElement('button');
  btnAccept.className = 'btn-approve';
  btnAccept.textContent = 'Valider tout';
  btnAccept.style.cssText = 'font-size:12px;padding:4px 10px;';
  btnAccept.title = 'Accepter tous les films de ce cinéaste qui ont une suggestion TMDB';

  var btnHold = document.createElement('button');
  btnHold.className = 'btn-reject';
  btnHold.style.cssText = 'font-size:12px;padding:4px 10px;';
  if(_tmdbFilter === 'en_attente'){
    btnHold.textContent = 'Remettre à valider';
    btnHold.title = 'Remettre tous les films de ce cinéaste dans la liste "À valider"';
  } else {
    btnHold.textContent = 'Mettre en attente';
    btnHold.title = 'Déplacer ce cinéaste dans la liste "En attente" pour y revenir plus tard';
  }
  ...
  header.addEventListener('click', function(e){
    ...
    if(body.style.display === 'none'){
      if(!bodyRendered){
        films.forEach(function(row){ body.appendChild(buildTmdbMatchCard(row)); });
        bodyRendered = true;
      }
  ...
  btnAccept.addEventListener('click', function(e){
    e.stopPropagation();
    var ids = films.filter(function(f){ return f.tmdb_id; }).map(function(f){ return f.id; });
    if(!ids.length){ showAdminNotice('Aucune suggestion TMDB pour ' + directorName + '.', false); return; }
    btnAccept.disabled = true;
    tcWithRetryTimeout(function(){
      return sb.from('film_tmdb_matches').update({ statut: 'manuel' }).in('id', ids).select('id', { count: 'exact', head: true });
    }).then(function(res){
      showAdminNotice(directorName + ' : ' + (res && res.count != null ? res.count : ids.length) + ' film(s) validé(s).', true);
      loadTmdbMatches();
    }).catch(function(err){
  ...
  btnHold.addEventListener('click', function(e){
    e.stopPropagation();
    var newStatut = _tmdbFilter === 'en_attente' ? 'a_valider' : 'en_attente';
    var ids = films.map(function(f){ return f.id; });
    btnHold.disabled = true;
    tcWithRetryTimeout(function(){
      return sb.from('film_tmdb_matches').update({ statut: newStatut }).in('id', ids).select('id', { count: 'exact', head: true });
    }).then(function(){
      loadTmdbMatches();
    }).catch(function(err){
      showAdminNotice('Erreur : ' + (err && err.message ? err.message : err), false);
...
async function updateTmdbMatchStatus(id, fields){
  return tcWithRetryTimeout(function(){
    return sb.from('film_tmdb_matches').update(fields).eq('id', id).select('id');
  });
}

function buildTmdbMatchCard(row){
  var card = document.createElement('div');
  card.className = 'submission-card ' + (row.statut || 'a_valider');

  var statusLabels = { auto: 'Associé (auto)', a_valider: 'À valider', manuel: 'Validé manuellement', rejete: 'Rejeté' };
  var suggestion = row.tmdb_titre
    ? escapeHtml(row.tmdb_titre) + (row.tmdb_annee ? ' (' + escapeHtml(row.tmdb_annee) + ')' : '') + ' — score ' + Math.round((row.score || 0) * 100) + '%'
    : 'Aucune suggestion';
  ...
  header.innerHTML = '<div class="submission-info">'
    + '<div class="submission-names">'
    + '<span class="submission-cineaste-nom">' + escapeHtml(row.cineaste_nom) + '</span>'
    + '<span class="submission-names-sep">→</span>'
    + '<span class="submission-contrib">' + escapeHtml(row.titre_saisi) + (row.annee_saisie ? ' (' + escapeHtml(row.annee_saisie) + ')' : '') + '</span>'
    + '</div>'
    + '<div class="submission-meta">' + suggestion + '</div>'
    + '</div>'
    + '<span class="submission-status ' + (row.statut || 'a_valider') + '">' + (statusLabels[row.statut] || row.statut) + '</span>';

  var body = document.createElement('div');
  body.className = 'submission-body';

  var actionsHtml = '<div class="submission-actions" style="flex-wrap:wrap;">';
  if(row.tmdb_id && row.statut !== 'manuel'){
    actionsHtml += '<button class="btn-approve" data-action="accept" data-id="' + row.id + '">Accepter la suggestion</button>';
  }
  if(row.statut !== 'rejete'){
    actionsHtml += '<button class="btn-reject" data-action="reject" data-id="' + row.id + '">Rejeter</button>';
  }
  if(row.statut !== 'a_valider'){
    actionsHtml += '<button class="btn-reject" data-action="reset" data-id="' + row.id + '">Remettre à revoir</button>';
  }
  actionsHtml += '</div>'
    + '<div class="tmdb-manual-search" style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">'
    + '<input type="text" class="tmdb-search-input" value="' + escapeHtml(row.titre_saisi) + '" style="flex:1;min-width:180px;padding:6px 10px;border-radius:6px;border:1px solid var(--border,#555);background:var(--bg,#111);color:inherit;font-size:13px;">'
    + '<input type="text" class="tmdb-search-year" value="' + escapeHtml(row.annee_saisie || '') + '" placeholder="Année" style="width:80px;padding:6px 10px;border-radius:6px;border:1px solid var(--border,#555);background:var(--bg,#111);color:inherit;font-size:13px;">'
    + '<button class="tmdb-search-btn" style="padding:6px 14px;border-radius:6px;background:var(--rouge,#A33025);color:#fff;border:none;cursor:pointer;font-size:13px;">Rechercher sur TMDB</button>'
    + '</div>'
    + '<div class="tmdb-search-results" style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap;"></div>'
    + '<div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">'
    + '<span style="font-size:12px;color:var(--text-muted,#888);">ID TMDB direct (ex: 11 depuis themoviedb.org/movie/<b>11</b>) :</span>'
    + '<input type="number" class="tmdb-manual-id" placeholder="ID" min="1" style="width:100px;padding:6px 10px;border-radius:6px;border:1px solid var(--border,#555);background:var(--bg,#111);color:inherit;font-size:13px;">'
    + '<button class="tmdb-manual-id-btn" style="padding:6px 14px;border-radius:6px;background:var(--vert,#1a6b3a);color:#fff;border:none;cursor:pointer;font-size:13px;">Valider cet ID</button>'
    + '<span class="tmdb-manual-id-status" style="font-size:12px;"></span>'
    + '</div>';
  body.innerHTML = actionsHtml;

  header.addEventListener('click', function(){ body.classList.toggle('visible'); });

  var acceptBtn = body.querySelector('[data-action="accept"]');
  if(acceptBtn) acceptBtn.addEventListener('click', function(e){
    e.stopPropagation();
    updateTmdbMatchStatus(row.id, { statut: 'manuel' }).then(function(){ showAdminNotice('Correspondance validée.', true); loadTmdbMatches(); });
  });
  var rejectBtn = body.querySelector('[data-action="reject"]');
  if(rejectBtn) rejectBtn.addEventListener('click', function(e){
    e.stopPropagation();
    updateTmdbMatchStatus(row.id, { statut: 'rejete' }).then(function(){ showAdminNotice('Film marqué comme rejeté.', true); loadTmdbMatches(); });
  });
  var resetBtn = body.querySelector('[data-action="reset"]');
  if(resetBtn) resetBtn.addEventListener('click', function(e){
    e.stopPropagation();
    updateTmdbMatchStatus(row.id, { statut: 'a_valider' }).then(function(){ showAdminNotice('Film remis à revoir.', true); loadTmdbMatches(); });
  });

  var manualIdBtn = body.querySelector('.tmdb-manual-id-btn');
  manualIdBtn.addEventListener('click', function(e){
    e.stopPropagation();
    var idVal = parseInt(body.querySelector('.tmdb-manual-id').value, 10);
    var statusEl = body.querySelector('.tmdb-manual-id-status');
    if(!idVal || idVal < 1){ statusEl.textContent = 'ID invalide.'; return; }
    statusEl.textContent = 'Vérification…';
    tmdbFetchJson('/movie/' + idVal).then(function(m){
      if(!m || !m.id){ statusEl.textContent = 'Film introuvable sur TMDB.'; return; }
      updateTmdbMatchStatus(row.id, {
        tmdb_id: m.id, tmdb_titre: m.title, tmdb_annee: m.release_date ? m.release_date.slice(0,4) : null,
        statut: 'manuel'
      }).then(function(){ showAdminNotice('"' + m.title + '" associé manuellement.', true); loadTmdbMatches(); });
    }).catch(function(){ statusEl.textContent = 'Erreur TMDB.'; });
  });

  var searchBtn = body.querySelector('.tmdb-search-btn');
  searchBtn.addEventListener('click', function(e){
    e.stopPropagation();
    var q = body.querySelector('.tmdb-search-input').value.trim();
    var y = body.querySelector('.tmdb-search-year').value.trim();
    var resultsEl = body.querySelector('.tmdb-search-results');
    if(!q) return;
    resultsEl.innerHTML = 'Recherche…';
    tmdbFetchJson('/search/movie', { query: q, year: y || undefined }).then(function(data){
      var results = (data && data.results) || [];
      resultsEl.innerHTML = '';
      if(!results.length){ resultsEl.textContent = 'Aucun résultat.'; return; }
      results.slice(0, 8).forEach(function(m){
        var item = document.createElement('div');
        item.style.cssText = 'width:90px;cursor:pointer;text-align:center;font-size:11px;';
        item.innerHTML = (m.poster_path
          ? '<img src="https://image.tmdb.org/t/p/w92' + m.poster_path + '" style="width:90px;height:135px;object-fit:cover;border-radius:4px;" alt="">'
          : '<div style="width:90px;height:135px;background:var(--bg-subtle);border-radius:4px;"></div>')
          + '<div style="margin-top:4px;">' + escapeHtml(m.title) + (m.release_date ? ' (' + m.release_date.slice(0,4) + ')' : '') + '</div>';
        item.addEventListener('click', function(){
          updateTmdbMatchStatus(row.id, {
            tmdb_id: m.id, tmdb_titre: m.title, tmdb_annee: m.release_date ? m.release_date.slice(0,4) : null,
            statut: 'manuel'
          }).then(function(){ showAdminNotice('Correspondance choisie manuellement.', true); loadTmdbMatches(); });
        });
        resultsEl.appendChild(item);
      });
    }).catch(function(err){
      resultsEl.textContent = 'Erreur TMDB : ' + (err && err.message ? err.message : err);
    });
  });
...
// ═══════════════════════════════════════════════════════════════
// 50 FILMS LES PLUS VUS – GESTION DES EXCLUSIONS DU CLASSEMENT
// ═══════════════════════════════════════════════════════════════
function showRankingFilmsError(msg){
  var el = document.getElementById('ranking-films-error');
  if(!el) return;
  el.textContent = msg;
  el.style.display = msg ? 'block' : 'none';
}

async function _adminLoadTopFilmsRows(offset, pageSize){
  var res = await tcWithRetryTimeout(function(){
    return sb.from('film_tmdb_matches')
      .select('tmdb_id, tmdb_titre, tmdb_annee, exclu_classement')
  ...
  if(rows.length === pageSize){
    var more = await _adminLoadTopFilmsRows(offset + pageSize, pageSize);
    return rows.concat(more);
  }
  return rows;
}

async function loadRankingFilms(){
  var listEl = document.getElementById('ranking-films-list');
  var refreshBtn = document.getElementById('btn-ranking-films-refresh');
  if(!listEl) return;
  listEl.innerHTML = '<div class="empty-state">Chargement…</div>';
  showRankingFilmsError('');
  if(refreshBtn) refreshBtn.disabled = true;

  try {
    var rows = await _adminLoadTopFilmsRows(0, 1000);

    // Grouper par tmdb_id : compter les non-exclus, détecter les exclus
    var counts = {}, meta = {};
    rows.forEach(function(row){
      var id = row.tmdb_id;
      if(!meta[id]) meta[id] = { tmdb_titre: row.tmdb_titre, tmdb_annee: row.tmdb_annee, exclu: false };
      if(row.exclu_classement){ meta[id].exclu = true; }
      else { counts[id] = (counts[id] || 0) + 1; }
    });

    var included = Object.keys(counts)
      .map(function(id){ return { tmdb_id: Number(id), tmdb_titre: meta[id].tmdb_titre, tmdb_annee: meta[id].tmdb_annee, nb: counts[id], exclu: false }; })
      .sort(function(a, b){ return b.nb - a.nb; });

    var excluded = Object.keys(meta).filter(function(id){ return meta[id].exclu && !counts[id]; })
      .map(function(id){ return { tmdb_id: Number(id), tmdb_titre: meta[id].tmdb_titre, tmdb_annee: meta[id].tmdb_annee, nb: 0, exclu: true }; })
      .sort(function(a, b){ return (a.tmdb_titre||'').localeCompare(b.tmdb_titre||'', 'fr'); });

    var all = included.concat(excluded);

    listEl.innerHTML = '';
    if(!all.length){
      listEl.innerHTML = '<div class="empty-state">Aucun film validé (statut auto ou manuel) dans la base.</div>';
      return;
    }
    ...
    all.forEach(function(f){
      if(!f.exclu) rank++;
      var row = document.createElement('div');
      row.style.cssText = 'display:flex;align-items:center;gap:12px;padding:9px 4px;border-bottom:1px solid var(--border,#333);';
      if(f.exclu) row.style.opacity = '0.4';

      var rankEl = document.createElement('div');
      rankEl.style.cssText = 'width:36px;text-align:right;font-size:15px;font-weight:700;color:var(--brun-pale);flex-shrink:0;';
      rankEl.textContent = f.exclu ? '—' : rank;

      var chk = document.createElement('input');
      chk.type = 'checkbox';
      chk.checked = !f.exclu;
      chk.style.cssText = 'width:16px;height:16px;flex-shrink:0;cursor:pointer;';
      chk.title = f.exclu ? 'Réintégrer au classement' : 'Exclure du classement';

      var info = document.createElement('div');
      info.style.cssText = 'flex:1;min-width:0;';
      var anneeHtml = f.tmdb_annee ? ' <span style="font-size:13px;opacity:.5">('+escapeHtml(String(f.tmdb_annee))+')</span>' : '';
      info.innerHTML = '<span style="font-size:15px;font-weight:600;">'+escapeHtml(f.tmdb_titre||'—')+anneeHtml+'</span>';

      var nbEl = document.createElement('div');
      nbEl.style.cssText = 'font-size:13px;color:var(--brun-pale);flex-shrink:0;white-space:nowrap;';
      nbEl.textContent = f.exclu ? 'exclu' : (f.nb + ' top' + (f.nb > 1 ? 's' : ''));

      var currentF = f;
      chk.addEventListener('change', async function(){
        chk.disabled = true;
        var exclu = !chk.checked;
        try {
          var res = await tcWithRetryTimeout(function(){
            return sb.from('film_tmdb_matches')
              .update({ exclu_classement: exclu })
              .eq('tmdb_id', currentF.tmdb_id)
              .in('statut', ['auto', 'manuel']);
          });
          if(res.error){
            showRankingFilmsError('Erreur : ' + friendlyError(res.error));
            chk.checked = !exclu;
          } else {
            showAdminNotice(exclu ? '✓ Film exclu du classement.' : '✓ Film réintégré au classement.', true);
            row.style.opacity = exclu ? '0.4' : '';
            nbEl.textContent = exclu ? 'exclu' : nbEl.textContent;
          }
        } catch(err){
          showRankingFilmsError('Erreur inattendue : ' + friendlyError(err));
          chk.checked = !exclu;
        } finally {
          chk.disabled = false;
        }
      ...
      row.appendChild(nbEl);
      listEl.appendChild(row);
    });

  } catch(err){
    showRankingFilmsError('Erreur de chargement : ' + (err && err.message ? err.message : err));
    listEl.innerHTML = '';
  } finally {
...
document.addEventListener('DOMContentLoaded', function(){
  var refreshBtn = document.getElementById('btn-ranking-films-refresh');
  if(refreshBtn) refreshBtn.addEventListener('click', loadRankingFilms);
});
