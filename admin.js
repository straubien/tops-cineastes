  ...
  try{
    ...
    var res = await tcWithRetryTimeout(function(){
      return sb.from('submissions').insert({
        ...
        reviewed_by: currentContributor ? currentContributor.id : null // admin ayant validé
      }).select('id');
...
async function updateSubmissionWithEdit(id, s, type, editedText){
  ...
  var res = await tcWithRetryTimeout(function(){
    return sb.from('submissions')
      .update({ status: 'approved', parsed_json: newParsedJson, reviewed_at: new Date().toISOString(), reviewed_by: currentContributor ? currentContributor.id : null })
      .eq('id', id)
      .select('id');
  });
...
async function updateSubmission(id, status, submission){
  showSubmissionsError('');
  var update = { status: status, reviewed_at: new Date().toISOString(), reviewed_by: currentContributor ? currentContributor.id : null }; // date + admin de validation/revue
