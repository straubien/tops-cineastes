...
async function renderDashboard(force){
  ...
  // Cinéastes et tops dans l'index : calculés depuis la table cineastes,
  // à l'identique de l'onglet Statistiques (nombre de cinéastes = DATA.cineastes.length ;
  // nombre de tops = somme des tops_contributeurs). Les deux pages affichent ainsi
  // les mêmes valeurs.
  var _totalCineastes = DATA.cineastes.length;
  var _totalTops = DATA.cineastes.reduce(function(s, c){ return s + ((c.tops_contributeurs || []).length); }, 0);
