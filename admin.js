...
document.querySelectorAll('.tab').forEach(function(tab){
  tab.addEventListener('click', function(){
    ...
    if(tab.getAttribute('data-tab') === 'courants-gestion') loadCourantCatalogue();
  });
});
...
async function loadCourantCatalogue(){
  ...
  try {
    res = await tcWithRetryTimeout(function(){
      return sb.from('courants').select('id, nom_fr, nom_en').order('nom_fr', { ascending: true });
    });
  } catch(err){
    if(errEl){ errEl.textContent = 'Erreur de chargement : ' + (err && err.message ? err.message : err); errEl.style.display = 'block'; }
    listEl.innerHTML = '';
...
function renderCourantCatalogue(rows){
  ...
  rows.forEach(function(row){
    ...
    card.innerHTML = '<div class="submission-names">'
      + '<span class="submission-cineaste-nom">' + escapeHtml(row.nom_fr) + '</span>'
      + '</div>';

    var enWrap = document.createElement('div');
    enWrap.style.cssText = 'display:flex;gap:8px;align-items:center;margin-top:8px;';
    ...
    saveBtn.addEventListener('click', function(){
      saveCourantTranslation(row.id, enInput.value.trim(), saveBtn);
    });
    enWrap.appendChild(enInput);
    enWrap.appendChild(saveBtn);
...
async function saveCourantTranslation(id, nomEn, btn){
  if(!nomEn){ showAdminNotice('Le nom anglais ne peut pas être vide.', false); return; }
  if(btn) btn.disabled = true;
  var res;
  try {
    res = await tcWithRetryTimeout(function(){
      return sb.from('courants').update({ nom_en: nomEn }).eq('id', id).select('id');
    });
  ...
  showAdminNotice('Traduction enregistrée.', true);
}
...
async function addCourantToCatalogue(){
  ...
  try {
    res = await tcWithRetryTimeout(function(){
      return sb.from('courants').insert({ nom_fr: nomFr, nom_en: nomEn }).select('id');
    }, { retries: 0 });
  } catch(err){
