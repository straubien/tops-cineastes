...
// Ajoute le courant proposé dans la première des 3 colonnes courant/courant2/courant3
// du cinéaste qui soit libre, sans doublon, puis marque la proposition comme validée.
async function approveCourant(row, apprBtn, rejBtn){
  ...
  var valeurs = slots.map(function(k){ return (cineasteRes.data[k] || '').trim(); });
  var dejaPresent = valeurs.some(function(v){ return v && v.toLowerCase() === row.courant.toLowerCase(); });

  if(dejaPresent){
    // Rien à modifier côté cinéaste, on passe directement à la validation de la proposition.
  } else {
    var slotLibre = valeurs.indexOf('');
    if(slotLibre === -1){
    ...
    var maj = {};
    maj[slots[slotLibre]] = row.courant;
    var updRes;
    try {
