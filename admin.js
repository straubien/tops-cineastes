...
function buildSubmissionCard(s, container){
  ...
  if(s.status === 'pending'){
    ...
    rejectBtn.addEventListener('click', function(e){
      e.stopPropagation();
      showConfirmModal(t('adm_confirm_reject', contribName), function(){
    ...
    if(p.status === 'pending'){
      ...
      approveBtn.addEventListener('click', function(){
        showConfirmModal(t('adm_confirm_prop_approve', nomComplet), function(){
          updateProposal(p.id, 'approved', approveBtn, rejectBtn);
        });
      });
      rejectBtn.addEventListener('click', function(){
        showConfirmModal(t('adm_confirm_prop_reject', nomComplet), function(){
          updateProposal(p.id, 'rejected', approveBtn, rejectBtn);
        });
      });
...
function renderFlagsAdmin(){
  ...
  // CORRECTIF BUG C : suppression de la limite à 200 — tous les cinéastes sont affichés
