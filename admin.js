...
function buildSubmissionCard(s, container){
  ...
  if(s.status === 'pending'){
    ...
    approveBtn.addEventListener('click', function(e){
      ...
      // GARDE-FOU DOUBLON : si un top approuvé existe déjà pour ce couple
      // (contributeur, cinéaste), on remplace la confirmation habituelle par un
      // avertissement explicite exigeant une validation consciente de l'admin.
      var confirmMsg;
      if(isTopModification(s)){
        confirmMsg = t('adm_confirm_dup', [contribName, cineasteTxt]);
      } else {
        confirmMsg = (type === 'top' && cineasteTxt)
