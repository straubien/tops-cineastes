...
async function runTmdbMatchingBatch(){
  ...
  try {
    ...
    var cineasteMeta = {};
    var cinRes = await tcWithRetryTimeout(function(){ return sb.from('cineastes').select('nom, naissance, tmdb_person_id'); });
    if(cinRes.error) throw new Error(cinRes.error.message);
    (cinRes.data || []).forEach(function(c){ cineasteMeta[normApo(c.nom)] = c; });
