...
async function updateProposal(id, status, approveBtn, rejectBtn){
  ...
  try {
    ...
    res = await tcWithRetryTimeout(function(){ return sb.from('cineaste_proposals').update({ status: status }).eq('id', id).select('id'); });
  } catch(err){
  ...
  showAdminNotice(status === 'approved' ? '✓ Proposition approuvée et prise en compte.' : '✓ Proposition rejetée et prise en compte.', true);
  loadProposals();
}
