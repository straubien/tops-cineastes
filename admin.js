...
document.querySelectorAll('[data-tmdb-filter]').forEach(function(btn){
  btn.addEventListener('click', function(){
    ...
    if(avalControls) avalControls.style.display = _tmdbFilter === 'a_valider' ? 'flex' : 'none';
    loadTmdbMatches();
  });
});
...
function tmdbNormTitle(s){
  ...
  n = n.replace(/^(the|a|an|le|la|les|l|un|une|der|die|das|el|lo|los|las|il|un')\s+/, '');
...
async function resolveCineasteTmdbPersonId(cineasteNom, naissanceAnnee){
  ...
  var best = results[0];
  if(naissanceAnnee && results.length > 1){
    var withYear = results.filter(function(r){ return r.known_for_department === 'Directing'; });
    if(withYear.length) best = withYear[0];
  }
...
// Récupère (avec cache mémoire) tous les titres connus (toutes langues) d'un film.
async function tmdbGetAllTitles(movie){
  ...
  try {
    ...
    translations.forEach(function(tr){
      if(tr.data && tr.data.title) titles.push(tr.data.title);
    });
  } catch(e){ /* best-effort : on garde au moins title/original_title */ }
  await tmdbSleep(120); // ménage le quota TMDB
...
async function tmdbMatchFilm(titreSaisi, anneeSaisie, filmography){
  ...
  for(var i = 0; i < filmography.length; i++){
    ...
    var titles = await tmdbGetAllTitles(movie);
    var score = 0;
    ...
    if(anneeSaisie && movie.annee){
      ...
      if(!isNaN(diff)){
        if(diff <= 1) score = Math.min(1, score + 0.05);
        else if(diff > 1) score = Math.max(0, score - 0.35);
      }
    }
...
var _tmdbFilterLabels = { a_valider: 'À valider', auto: 'Associés auto', manuel: 'Validés manuellement', rejete: 'Rejetés' };

async function updateTmdbFilterCounts(){
  var statuts = ['a_valider', 'auto', 'manuel', 'rejete'];
...
async function loadTmdbMatches(){
  ...
  try {
    res = await tcWithRetryTimeout(function(){
      ...
      if(_tmdbFilter === 'a_valider'){
        if(_tmdbAvalSub === 'with_match') q = q.not('tmdb_id', 'is', null);
        else if(_tmdbAvalSub === 'no_match') q = q.is('tmdb_id', null);
        // Score décroissant : les plus proches du seuil en premier (plus faciles à valider)
