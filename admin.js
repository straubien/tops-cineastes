...
async function runTmdbMatchingBatch(){
  ...
  try {
    ...
    async function loadDecided(offset, pageSize){
      var res = await tcWithRetryTimeout(function(){
        return sb.from('film_tmdb_matches').select('cineaste_nom, titre_saisi, annee_saisie, statut')
          .in('statut', ['manuel', 'rejete']).range(offset, offset + pageSize - 1);
      });
      if(res.error) throw new Error(res.error.message);
      var rows = res.data || [];
    ...
    // DEBUG — noms sans correspondance dans cineastes (à retirer après diagnostic)
    console.log('DEBUG byCineaste total:', totalCineastes);
    console.log('DEBUG sans match cineasteMeta:', cineasteNoms.filter(function(cn){ return !cineasteMeta[cn]; }).sort());
    var upserts = [];
