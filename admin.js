...
async function onLogin(user){
  currentUser = user;
  var res = await tcWithRetryTimeout(function(){ return sb.from('contributors').select('*').eq('auth_id', user.id).single(); });
...
async function renderDashboard(force){
  ...
  document.getElementById('dash-pending-count').textContent = t('adm_pending_count', nPending);
  ...
  if(jsonWarnings.length){
    ...
    if(warnEl) warnEl.textContent = t('adm_json_warning', jsonWarnings.join(', '));
  }
...
async function loadSubmissions(){
  ...
  var countRes = await sb.from('submissions').select('id', { count: 'exact', head: true });
...
async function legacyTopExists(submission, parsedJson){
  ...
  var res = await sb.from('tops').select('id').eq('contributor_id', contributorId).eq('cineaste_nom', cineaste).limit(1);
...
async function removeLegacyTopDuplicate(submission, parsedJson){
  ...
  var res = await sb.from('tops').delete().eq('contributor_id', contributorId).eq('cineaste_nom', cineaste).select('id');
  if(res.error){
...
async function updateProposal(id, status, approveBtn, rejectBtn){
  ...
  if(res.error){ alert(t('adm_err_prefix') + friendlyError(res.error)); approveBtn.disabled = false; rejectBtn.disabled = false; return; }
  if(!res.data || !res.data.length){ alert(t('adm_prop_rights')); approveBtn.disabled = false; rejectBtn.disabled = false; return; }
  loadProposals();
}
...
async function loadCommentsAdmin(){
  ...
  try {
    ...
    if(res.error){
      errEl.textContent = t('adm_cm_load_err') + friendlyError(res.error);
      errEl.style.display = 'block';
      container.innerHTML = '';
      return;
    }
    ...
    errEl.textContent = t('adm_err_load') + friendlyError(err);
    errEl.style.display = 'block';
    container.innerHTML = '';
  }
}
...
async function deleteCommentAdmin(id, delBtn){
  if(!confirm(t('adm_cm_confirm'))) return;
  delBtn.disabled = true;
  var res = await sb.from('comments').delete().eq('id', id).select('id');
  if(res.error){ alert(t('adm_err_prefix') + friendlyError(res.error)); delBtn.disabled = false; return; }
  if(!res.data || !res.data.length){ alert(t('adm_cm_err_rights')); delBtn.disabled = false; return; }
  loadCommentsAdmin();
}
...
function renderFlagsAdmin(){
  ...
  if(rows.length === 200){
    ...
    infoTd.colSpan = 3;
    infoTd.style.color = '#777';
    infoTd.style.fontSize = '13px';
    infoTd.textContent = t('adm_flags_limit');
