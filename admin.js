  ...
  try {
    ...
    async function loadAllTops(offset, pageSize){
      ...
      rows.forEach(function(row){
        ...
        if(!byCineaste[row.cineaste_nom]) byCineaste[row.cineaste_nom] = {};
        row.films.forEach(function(f){
          if(!f || !f.titre) return;
          var key = f.titre.trim() + '|||' + (f.annee || '').toString().trim();
          byCineaste[row.cineaste_nom][key] = { titre: f.titre.trim(), annee: (f.annee || '').toString().trim() };
        });
      });
    ...
    async function loadAllApprovedSubmissions(offset, pageSize){
      ...
      rows.forEach(function(row){
        ...
        if(!byCineaste[pj.cineaste]) byCineaste[pj.cineaste] = {};
        pj.films.forEach(function(f){
          if(!f || !f.titre) return;
          var key = f.titre.trim() + '|||' + (f.annee || '').toString().trim();
          byCineaste[pj.cineaste][key] = { titre: f.titre.trim(), annee: (f.annee || '').toString().trim() };
        });
      });
    ...
    async function loadJsonTops(url){
      try {
        ...
        data.tops.forEach(function(top){
          var cinNom = top.cineaste;
          if(!cinNom || !Array.isArray(top.films)) return;
          if(!byCineaste[cinNom]) byCineaste[cinNom] = {};
          top.films.forEach(function(f){
            if(!f || !f.titre) return;
            var key = f.titre.trim() + '|||' + (f.annee || '').toString().trim();
            byCineaste[cinNom][key] = { titre: f.titre.trim(), annee: (f.annee || '').toString().trim() };
          });
        });
      } catch(e){
    ...
    async function loadDecided(offset, pageSize){
      ...
      rows.forEach(function(r){
        decided[r.cineaste_nom + '|||' + r.titre_saisi + '|||' + (r.annee_saisie || '')] = true;
      });
    ...
    // normApo : normalise les variantes d'apostrophe (typographique ' U+2019
    // vs droite ' U+0027) pour que L'HERBIER et L'HERBIER se correspondent.
