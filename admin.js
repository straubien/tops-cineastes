...
async function renderStats(){
  ...
  var countById = {};
  (cineastesRes || []).forEach(function(c){
  ...
  var courantsList = [];
  var categoriesList = [];
  (courantsRows || []).forEach(function(row){
    var item = { nom: row.nom_fr, count: countById[row.id] || 0 };
    if(row.type === 'categorie') categoriesList.push(item);
    else courantsList.push(item);
  });

  renderStatsChart(courantsChartEl, courantsList);
  renderStatsChart(categoriesChartEl, categoriesList);
}

function renderStatsChart(containerEl, items){
  ...
  items.forEach(function(it){
    ...
    topRow.className = 'stats-bar-toprow';
    ...
    count.textContent = it.count.toLocaleString('fr-FR') + (it.count > 1 ? ' fiches' : ' fiche');
    ...
    fill.style.width = Math.round((it.count / maxCount) * 100) + '%';
