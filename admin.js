...
async function removeCourantFromFiche(c, slotKey, btn){
  ...
  c[slotKey] = null;
  await removeCourantActuAndNotif(c.nom, courantId);
  showAdminNotice('Courant retiré de la fiche de « ' + c.nom + ' ».', true);
  renderCourantFicheResult(c.nom);
  refreshStatsIfActive();
}

// Nettoie, best-effort, la trace laissée par la validation de ce courant pour
// ce cinéaste : l'entrée « Actualités » (courant_proposals approuvée, source
// du message affiché) et la notification envoyée au cinéphile proposeur.
// Un échec ici ne doit jamais bloquer le retrait du courant lui-même — mais
// un échec silencieux (ex. suppression bloquée par une policy RLS Supabase
// qui autorise le SELECT sans autoriser le DELETE : la requête ne renvoie
// alors aucune erreur, elle supprime simplement 0 ligne) doit rester visible
// pour l'admin plutôt que de disparaître dans un catch vide.
async function removeCourantActuAndNotif(cineasteNom, courantId){
  if(!courantId) return;
  var courantRow = _TC_COURANTS_CACHE ? _TC_COURANTS_CACHE[courantId] : null;
  var nomFr = courantRow ? courantRow.nom_fr : null;
  if(!nomFr) return;

  try {
    var propRes = await tcWithRetryTimeout(function(){
      return sb.from('courant_proposals').select('id, courant').eq('cineaste_nom', cineasteNom).eq('status', 'approved');
    });
    if(propRes && !propRes.error && propRes.data){
      var propIds = propRes.data.filter(function(row){ return row.courant === nomFr; }).map(function(row){ return row.id; });
      if(propIds.length){
        var propDelRes = await tcWithRetryTimeout(function(){ return sb.from('courant_proposals').delete().in('id', propIds).select('id'); });
        if(propDelRes.error){
          showAdminNotice('Courant retiré, mais la suppression de la proposition liée (courant_proposals) a échoué : ' + propDelRes.error.message, false);
        } else if(!propDelRes.data || propDelRes.data.length < propIds.length){
          showAdminNotice('Courant retiré, mais la proposition liée (courant_proposals) n\'a pas pu être supprimée — vérifiez la policy RLS DELETE de cette table dans Supabase.', false);
        }
      }
    }
  } catch(e){
    showAdminNotice('Courant retiré, mais le nettoyage de courant_proposals a échoué : ' + (e && e.message ? e.message : e), false);
  }

  try {
    var notifRes = await tcWithRetryTimeout(function(){
      return sb.from('notifications').select('id, data').eq('type', 'proposal_courant_approved').eq('cineaste_nom', cineasteNom);
    });
    if(notifRes && !notifRes.error && notifRes.data){
      var notifIds = notifRes.data.filter(function(row){ return row.data && row.data.courant === nomFr; }).map(function(row){ return row.id; });
      if(notifIds.length){
        var notifDelRes = await tcWithRetryTimeout(function(){ return sb.from('notifications').delete().in('id', notifIds).select('id'); });
        if(notifDelRes.error){
          showAdminNotice('Courant retiré, mais la suppression de la notification liée a échoué : ' + notifDelRes.error.message, false);
        } else if(!notifDelRes.data || notifDelRes.data.length < notifIds.length){
          showAdminNotice('Courant retiré, mais la notification liée n\'a pas pu être supprimée — vérifiez la policy RLS DELETE de la table notifications dans Supabase.', false);
        }
      }
    }
  } catch(e){
    showAdminNotice('Courant retiré, mais le nettoyage des notifications a échoué : ' + (e && e.message ? e.message : e), false);
  }
}
