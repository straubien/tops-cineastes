...
const sb = createClient(TC_SUPABASE_URL, TC_SUPABASE_KEY, {
  auth: {
    flowType: 'implicit',
...
fetch('photos-tmdb.json').then(function(r){return r.json()}).then(function(data){
  PHOTOS_TMDB = data || {};
  PHOTOS_TMDB_NORM = {};
...
function lookupPhoto(nom){
  return PHOTOS_TMDB[nom] || PHOTOS_TMDB_NORM[normStr(nom)];
}
function buildAvatarHtml(photoVal){
  var path = Array.isArray(photoVal) ? (photoVal[0] || photoVal[1]) : photoVal;
  if(!path) return '<span class="sp-view-avatar sp-view-avatar-empty"></span>';
  return '<img class="sp-view-avatar" src="https://image.tmdb.org/t/p/w92'+path+'" alt="" loading="lazy" onerror="this.outerHTML=\'<span class=&quot;sp-view-avatar sp-view-avatar-empty&quot;></span>\'">';
}
...
(function(){
  ...
  var knownName = localStorage.getItem('tc-display-name');
  if(knownName){
  ...
  // Timeout de sécurité : si aucune session après 15s, afficher le login
...
async function onLogin(user){
  ...
  localStorage.setItem('tc-display-name', data.display_name);
...
// Charger les cinéastes depuis Supabase (paginé, db.max_rows plafonne à 1000 lignes/requête)
  ...
  return sb.from('cineastes').select('nom,fbid,url_facebook,duo,naissance,deces,vivant,tops_contributeurs')
    ...
    .then(function(res){
      ...
      if(rows.length === pageSize){
        return loadAllCineastesSubmit(offset + pageSize, pageSize).then(function(more){ return rows.concat(more); });
      }
      return rows;
    });
}

loadAllCineastesSubmit(0, 1000).then(function(rows){
  cineastesIndex = rows.map(function(c){ return c.nom; });
...
function renderViewList(containerId, items){
  var el = document.getElementById(containerId);
  if(!items || !items.length){ el.innerHTML = '<span class="sp-empty">'+t('sp_empty_item')+'</span>'; return; }
  el.innerHTML = '<div class="sp-view-list">' + items.map(function(n){
    var clean=n.replace(/,.*$/,'');
    ...
    var avatarHtml = buildAvatarHtml(lookupPhoto(n) || lookupPhoto(clean.trim()));
    return '<span class="sp-view-item" title="'+nom+'">'+avatarHtml+'</span>';
  }).join('') + '</div>';
}
...
function refreshProfilViews(){
  if(!currentContributor) return;
  renderViewList('sp-favoris-view', currentContributor.cineaste_coeur);
  renderViewList('sp-autres-view', currentContributor.cineaste_autres);
...
document.getElementById('btn-upload-avatar').addEventListener('click', async function(){
  ...
  try {
    ...
    var res = await sb.storage.from('avatars').upload(filename, file, {
      upsert: true,
...
function initProfil(){
  ...
  if(currentContributor.presentation)
    document.getElementById('presentation-textarea').value = currentContributor.presentation;
