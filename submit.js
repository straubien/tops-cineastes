...
function cinNomPrenomVariant(raw){
  // Convertit "Prénom NOM" en "NOM, Prénom" pour matcher le format des noms de la
  // table `cineastes` (colonne photo_tmdb). Note : les photos TMDB ne proviennent
  // plus d'un fichier photos-tmdb.json (supprimé) mais de Supabase.
...
// Portrait local (dossier /portraits) : fonction mutualisée dans utils.js
// (tcLocalPortraitPath). Elle est prioritaire sur les photos TMDB.
...
function makeLocalAvatar(n, fallbackToTmdb){
  ...
  if(!candidates.length){
    if(fallbackToTmdb){
      ...
      return makeTmdbAvatar(pv);
    }
    return makeEmptyAvatar();
  }
  var img = document.createElement('img');
  ...
  img.onerror = function(){
    if(candidates.length){ img.src = candidates.shift(); return; }
    if(fallbackToTmdb){
...
// ── INDEX CINÉASTES (alimente l'autocomplétion des cinéastes favoris) ─────────
var cineastesIndex = [];
...
document.getElementById('btn-upload-avatar').addEventListener('click', async function(){
  ...
  try {
    ...
    // par défaut (upsert:true → l'opération reste idempotente en cas de relance).
    var res = await tcWithRetryTimeout(function(){ return sb.storage.from('avatars').upload(filename, file, {
      upsert: true,
      contentType: file.type
    }); }, { timeoutMs: 40000 });
    if(res.error){ if(tcIsAuthError(res.error)) tcNotifyAuthExpired(); btn.disabled = false; btn.textContent = t('sp_avatar_publish'); alert(t('sp_err_save')+friendlyError(res.error)); return; }
