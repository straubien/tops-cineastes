...
async function submitTops(){
  ...
  try {
    ...
    insertRes = await tcWithRetryTimeout(function(){
    ...
    alert(t('sp_err_submission') + friendlyError(err));
    btn.disabled = false;
...
async function loadPrevSubmissions(){
  ...
  try {
    ...
    // Erreur réseau : NE PAS masquer la section (on conserve l'affichage
    // précédent). Nouvel essai à la prochaine ouverture. Cf. audit 1.7 :
    // une erreur ne doit pas être rendue comme « aucune soumission ».
    console.error('loadPrevSubmissions: échec réseau', err);
    return;
  }
  if(res && res.error){ console.error('loadPrevSubmissions:', res.error.message); return; }
  var data = res.data;
  ...
  data.forEach(function(s){
    ...
    saveBtn.addEventListener('click', async function(){
      ...
      try {
        ...
        res = await tcWithRetryTimeout(function(){ return sb.from('submissions').update({ parsed_json: newParsedJson, status: 'pending' }).eq('id', s.id).select('id'); });
      } catch(err){ alert(t('sp_err_err') + friendlyError(err)); saveBtn.disabled = false; saveBtn.textContent = t('sp_sauvegarder'); return; }
    ...
    delBtn.addEventListener('click', async function(){
      ...
      try {
        ...
        res = await tcWithRetryTimeout(function(){ return sb.from('submissions').delete().eq('id', s.id).select('id'); });
      } catch(err){ alert(t('sp_err_err') + friendlyError(err)); delBtn.disabled = false; return; }
...
document.getElementById('btn-upload-avatar').addEventListener('click', async function(){
  ...
  try {
    // Préfixe par l'id du contributeur : garantit l'unicité du nom de fichier
