...
//
// - Codes ISO 3166-1 alpha-2 (minuscules) : pays existants, drapeau résolu via flagsapi.com
...
var TC_ISO_COUNTRIES = {
  ...
  'hk':'Hong Kong','hn':'Honduras','hu':'Hongrie','in':'Inde','id':'Indonésie','iq':'Irak',
  ...
  'ug':'Ouganda','uz':'Ouzbékistan','pk':'Pakistan','ps':'Palestine','pa':'Panama','pg':'Papouasie-Nouvelle-Guinée',
...
// Mémoïsée : TC_ISO_COUNTRIES/TC_HISTORIC_LABELS ne changent pas en cours de
// session, alors que cette fonction est appelée à chaque cinéaste affiché
// dans l'onglet "Drapeaux" de l'admin (jusqu'à 2x par ligne) — sans cache,
// retrier ~270 entrées à chaque appel gelait la page pendant une recherche.
var _tcCountrySelectOptionsCache = null;
function tcCountrySelectOptions(){
  if(_tcCountrySelectOptionsCache) return _tcCountrySelectOptionsCache;
  ...
  _tcCountrySelectOptionsCache = { iso: iso, historic: historic };
  return _tcCountrySelectOptionsCache;
}
...
function tcFlagUrl(code){
  ...
  if(TC_ISO_COUNTRIES[code]) return 'https://flagsapi.com/' + code.toUpperCase() + '/flat/64.png';
  return null;
}
...
function tcFlagHtml(code, cssClass){
  ...
  var cls = cssClass || 'tc-flag';
  if(tcIsHistoricCode(code)) cls += ' ' + cls + '-historic';
  return '<img class="'+cls+'" src="'+url+'" alt="'+escapeHtml(label)+'" title="'+escapeHtml(label)+'" loading="lazy">';
}
